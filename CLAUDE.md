...
**Gewählt wird er im Geschosseditor** — beim Zeichnen und seit #75/#111 für mehrere ausgewählte
Wände gemeinsam im Sammel-Popup; Modul 1 führt ihn beim Neuaufbau unverändert mit, Modul 3 liest ihn
nur (kein UI in 1/3). Er hängt **nicht** am Core/an der
...
wird sie in Modul 1** und seit #111 zusätzlich im **Sammel-Editor des Geschosseditors** für mehrere
ausgewählte Wände gemeinsam — zwei **Bedienorte**, aber **eine** Quelle und **ein** Schreibweg: beide
setzen denselben Wert am selben Wandelement über denselben kanonischen Neuberechnungspfad. Wo der
Geschosseditor sie nicht ausdrücklich setzt, **führt er sie in `rechneWandelement()` unverändert
mit** (`geschossplan.html`, neben `wandtyp`/`abdichtung`) — ohne diese Zeile setzte jede Längen- oder
Höhenänderung eine gewählte F30 still auf F0 zurück, weil `buildWall()` das Wandelement neu erzeugt.
Vererbt wird nichts (weder Geschoss noch Projekt). Kanonische Werte und
...
`storage.js`. **Außerhalb** des Sammel-Popups hat der Editor dafür **kein Bedienelement** — nicht in der
Werkzeugleiste, nicht in der Wandliste, nicht auf der Bühne —, und die Zeichnung selbst liest
ausschließlich. Seit #111 setzt der **Sammel-Editor** (Popup „Gemeinsam bearbeiten…") sie für
mehrere ausgewählte Wände gemeinsam; das ist kein zweiter Schreibweg, sondern derselbe: er schreibt
je Wand über `rechneWandelement()` und `store.speichere`, genau wie Modul 1.
...
`start_axis_grid` ist seit #104 **unwirksamer Altbestand**: [V-5] ist durch [V-3]/[V-11] abgelöst,
der Core liest das Feld nicht mehr, und das Bedienelement in Modul 1 ist **ersatzlos
zurückgebaut** — es gibt keinen Schreibweg und keine Anzeige mehr (auch nicht im Zeichnungsblatt).
Das Feld bleibt im Datenmodell, damit gespeicherte Projekte unverändert lesbar sind; geschrieben
wird es nur noch vom unveränderten Rechenkern mit dessen Default `0`. Wer eine abweichende
Verteilung braucht, hat weiterhin **[V-9]** (`columns_grid`). **[V-3]** ist ein reiner *Positions*vorrang: muss für [V-2] ohnehin
...
`store.setzeProduktrolle(rolle, ids, elementId)` schreibt anhand der Rolle in den Abschnitt ihres
Eigentümers; eine fremde/unbekannte Rolle wird abgelehnt statt still einsortiert. Der optionale
dritte Parameter benennt die Wand — ohne ihn gilt die aktive. Genau darüber setzt der
**Sammel-Editor des Geschosseditors** (#111, Folgepaket) die Modul-1-Rollen für **mehrere**
ausgewählte Wände gemeinsam: ein zweiter **Bedienort**, aber **kein** zweiter Schreibweg und
**keine** zweite Rollen- oder Auswahllogik (Rollenliste, Gruppen und Produktoptionen kommen
ausschließlich aus `sembla-katalog.js`). Maßwirksame Rollen ziehen dort dieselbe Neurechnung nach
wie in Modul 1 ([Z-1]), damit im gespeicherten Wandelement nicht die alte Zerlegung stehen bleibt.
Seit #108 setzt der Sammel-Editor je Rolle **mehrere** Produkte — dieselbe Mehrfachauswahl, die
Modul 1 kennt; ein zweites Auswahlmodell entsteht dadurch nicht.
...
der unkalibrierte Plan liegt dafür vorläufig darunter, ohne Raster). **Aktiv ≠ ausgewählt** (s. u.), Endgriffe ändern die Länge, Mittelgriff/Körper die Lage, **R** dreht um 90°. Dazu **Bemaßen** (**D**) und **Fixieren** (**F**) über die sechs kanonischen Bezüge — die Achse folgt dem Bezug ([K-1]/[K-2]), Fixieren ist eine normale Bemaßung `von: null` je Achse ([K-4]) —, sichtbarer Widerspruch ([K-6]) und Redundanz ([K-7]), Längenmaß ([K-11]), **Doppelklick auf die Maßzahl** (öffnet aus #51 die Eingabe **an Ort und Stelle**, erkannt im **Zeigerstrom** statt am `dblclick`, schreibt aber weiter über `bemSetzen`) und **Undo/Redo**. Aus C4c dazu die **schwebende Bauteilliste** (Anzeige + Auswahl, kein Verortungsweg). Aus #50 (Paket 1) dazu: **kein** linker „Neue Wand"-Abschnitt mehr — Zielwahl, **Standard-Wandhöhe** und Wandtyp sind Parameter **des Werkzeugs** „Wand zeichnen", der Fang gehört zur Ansicht; je Wand mit Wandelement ein Knopf **„Planen"** in der Liste (aktiv setzen + Modul 1, gemeinsame `planeWand()`, kein Knopf bei verwaistem Eintrag). Aus #51 (Paket 2): **Inline-Eingabe des Maßwerts** an der Maßzahl (Enter/Escape/Blur, ungültig ⇒ keine Änderung + rotes Feld) und die **verschiebbare Maßzahl** (`text_mm`, reine Darstellung, Schwelle 3 px, Speichern erst bei `pointerup`). Aus #52: der Plan liegt **sofort** als Hintergrund (unkalibriert vorläufig 1 px = 1 mm, ohne 125-mm-Raster), **kalibriert wird auf der Bühne** („Maßstab aus Plan übernehmen“, Zoom/Pan bleiben nutzbar, kein Popup), und der **Rasterfang** ist ein allgemeiner Schalter für alle Wände, der **aus** startet. Aus #53 (Paket 3): **kein linkes Panel** mehr — obere **Werkzeugleiste** (Auswahl/Wand/Maß/Fix + Undo/Redo + Drehen, **ohne** „Plan verschieben“), untere **Ansichtsleiste** (Zoom, Fang, **Raster** und **Bemaßungen** als flüchtige Schalter, Zugang „Plan…“), rechte **Wandliste** und die **Planverwaltung als Blatt von unten** — der **einzige Uploadweg** der Suite (hochladen/ersetzen/entfernen/kalibrieren/Maßstab/Versatz/Plan verschieben), beim Kalibrieren auf Status und Abbruch verkleinert. Das **Referenzgeschoss ist entfallen**, und **Drehen** ist gesperrt, sobald eine Bemaßung unmittelbar an der Wand hängt. Die **Brandschutzklassifikation F0/F30** (#79) ist je Wand erkennbar: Kurztext am Wandende, für F30 zusätzlich **Schraffur** über der Wandfläche, dazu Kennfarbe, zwei Legendeneinträge mit dem Merkmal in Worten und die Angabe in der Wandliste — auch **schwarz-weiß** unterscheidbar. Gezeichnet wird sie **nur gelesen**; **gesetzt** wird sie seit #111 im **Sammel-Popup** „Gemeinsam bearbeiten…“ (s. u.) — außerhalb davon hat der Editor kein Bedienelement dafür. Die [K-8]-Zustandsfarbe bleibt unverändert, ein verwaister Eintrag bleibt **ohne** Angabe. Aus #75/#111 der **Sammel-Editor**: ab zwei ausgewählten Wänden **genau eine** Schaltfläche „Gemeinsam bearbeiten…“ und dahinter ein Popup mit **zehn** allgemeinen Wandmerkmalen (Wandhöhe — seit #136 millimetergenau frei —, Aktivierung der Ausgleichslage (#136), Windsituation, Brandschutzklasse, Stoßfugen-Abdichtung, Funktion der Vorder- und Rückseite, oberer Anschluss, Kopfblech-Modullänge, Überstand des Reststücks) **und — seit dem Folgepaket zu #111 — der Produktauswahl von Modul 1**: alle wählbaren Rollen aus `KAT.rollenVonModul(1)`, gegliedert nach `gruppe` in derselben Reihenfolge wie Modul 1, je Rolle die **Mehrfachauswahl** aus dem zugeordneten Katalog ([L-12]) — seit #108 dasselbe Häkchen-Dropdown wie in Modul 1 ([P-17]), es lassen sich also auch mehrere Stangenlängen ([Z-2]) oder Bodenblechlängen ([A-10]) gemeinsam setzen; die angekreuzte Menge **ersetzt** die bisherige Auswahl der Wand und wird nie mit ihr vereinigt. Je Merkmal und je Rolle ein Häkchen, sonst passiert nichts; verschiedene Ausgangswerte stehen als **gemischt** und werden nie still ersetzt (eine **eindeutige** Ausgangsmenge wird seit #108 vorbelegt, auch eine mehrfache); der Auswahlzustand des Popups lebt flüchtig in `GP.sammelWahl` und wird über **einen** delegierten Behandler am Kasten `#gp-sammel-rollen` bedient — wie `bindProdukte()` in Modul 1; „Übernehmen…“ nennt vorab Wandzahl, Zielwerte und je Rolle die Zielprodukte, schreibt je Wand über `rechneWandelement()`/`store.speichere` und `store.setzeProduktrolle(rolle, ids, wandId)` und ist **ein** Rückgängig-Schritt mit vollständigem Rollback — samt Produktauswahl, weil die Element-Momentaufnahmen `eingaben` mitführen (eine vorher **leere** Rolle steht darin ausdrücklich als `[]`, sonst könnte der zusammenführende Schreibweg sie nie wieder entfernen). Die fünf **maßwirksamen** Rollen (`rod_std`, `rod_rest`, `blech_boden`, `kupplung`, `spannplatte`) ziehen dieselbe Neurechnung nach, die Modul 1 nach [Z-1] fährt — über **denselben** `rechneWandelement()`-Pfad, mit den Eingängen aus `WA.vorspannVorgaben`/`KAT.produkteZuRolle`. **Seit #117 bildet jede Neurechnung diese Eingänge für alle fünf Stellen** aus der dann geltenden Produktauswahl der jeweiligen Wand (`vorspannEingaenge`) — nicht mehr nur für die angekreuzte Rolle, und auch dann, wenn nur ein Merkmal wie der Überstand angekreuzt ist; die angekreuzte Produktmenge liegt dabei über der gespeicherten Auswahl, für die übrigen Stellen gilt die gespeicherte. Damit steht im gespeicherten Wandelement dieselbe Zerlegung, die Modul 1 mit „Auslegen“ liefert (vorher blieb ein veralteter oder fehlender Eingangssatz stehen). Ein Maß, das sich aus der Auswahl nicht eindeutig ergibt, lässt sein Feld **weg** — dann bleibt der gespeicherte Eingang stehen —, und **ohne zugeordneten Katalog wird gar nichts abgeleitet**. Fehlt der zugeordnete Katalog oder steht ein gewähltes Produkt nicht darin, wird die **ganze** Sammeländerung benannt abgewiesen und nichts geschrieben. Geometrie- und positionsabhängige Angaben (Länge, Lage, Öffnungen, Durchbrüche, Staffelungen, Verzahnungen, manuelle Spannachsen, Zwischenspannpunkte), `eingaben.aufbau` (Modul 2), die Projektmappe und der Katalog selbst bleiben ausdrücklich **unberührt**; Modul-2-Rollen und nicht wählbare Beschaffungsrollen (`rod_sonder`, `blech_boden_sonder`) werden gar nicht angeboten. **Seit #130 liegt die Ableitung `ROLLE_RECHNUNG`/`vorspannEingaenge` als gemeinsamer Baustein in `sembla-wandanlage.js`** — der Editor delegiert, und derselbe Baustein speist den Read-only-Frischpfad (`wandelementAktualisiert`, der damit geschlossene Vorschlag C zu #120): Modul 4/7 und der zentrale Export leiten den vollen Vorspann-Eingangssatz (#92/#97) bei jeder Ausgabe frisch aus der Produktauswahl und dem zugeordneten Katalog ab, ein Fassungswechsel ([L-12]) wirkt also sofort auf die Darstellung. **⚠ Nachziehpunkt [P-6]:** die `einbauMass()`-Familie in Modul 1 bildet dieselben Werte weiterhin selbst (sie braucht die Statusgründe je Rolle); die Zahlen sind satzgleich, die Zusammenführung bleibt offen. Schreibt sonst **nur** Lage und Bemaßungen im Geschoss ([K-10]) — Maßstab/Versatz bleiben Plan-Ansichtsparameter ([L-9]), Planbild, Ansichtsschalter und der Popup-Zustand stehen in keinem Undo-Schritt |
| 1 | `wandplanung.html` | Wandhöhe, Öffnungen, Durchbrüche, Staffelung, Seiten, Auslegung (+ `sembla-engine.js`), **Abdichtung** ([A-6]) und die **Brandschutzklassifikation F0/F30** (#79, reine Planungskennzeichnung, Standard F0 — je Wand hier, für mehrere Wände gemeinsam im Sammel-Editor des Geschosseditors (#111): zwei Bedienorte, eine Quelle); die im Geschosseditor geführte Länge ist nur Anzeige. Schreibt die übrige Wandplanung und **Produkte dieser Wand** (Steine, Vorspannung, Anschluss inkl. getrennter Boden-/Kopfbleche, Fugen) → `eingaben.planung.produkte`. Für **mehrere** Wände gemeinsam setzt der Sammel-Editor des Geschosseditors dieselben Rollen (#111, Folgepaket) — zwei **Bedienorte**, **eine** Quelle und **ein** Schreibweg (`store.setzeProduktrolle`); die **Mehrfachauswahl** je Rolle ([A-10]/[Z-2]) gibt es seit #108 an **beiden** Bedienorten |
