...
// Der GEFUELLTE STANDARDKATALOG (#113, drittes Paket) ist der neueste Eintrag — er wird als
...
const STDKATALOG113 = EINTRAEGE[0];
ok("[#113] der gefuellte Standardkatalog ist der neueste Eintrag",
...
// Eintrag — er wird als einziger direkt ueber `EINTRAEGE[1]` geprueft; die bisherige Reihe
...
const BESCHBLOCK113 = EINTRAEGE[1];
...
// zweitneueste Eintrag — er wird als einziger direkt ueber `EINTRAEGE[3]` geprueft; die bisherige
...
const BLECHSTOSS91 = EINTRAEGE[3];
...
// viertneueste Eintrag — er wird als einziger direkt ueber `EINTRAEGE[5]` geprueft; die
...
// `EINTRAEGE[6]` geprueft; die bisherige Reihe rueckt geschlossen um eins nach hinten.
...
const DECKENZEIGEN = EINTRAEGE[6];
...
// aus #94) ist der neueste Eintrag — er wird als einziger direkt ueber `EINTRAEGE[7]` geprueft;
...
// chg-20260907-20) ist der neueste Eintrag — er wird als einziger direkt ueber `EINTRAEGE[8]`
...
const SAMMELMEHRFACH = EINTRAEGE[8];
...
const OHNE_SCHEIBE = EINTRAEGE[9];
...
// mit [A-3]/#97) sind der neueste Eintrag — er wird als einziger direkt ueber `EINTRAEGE[10]`
...
const ANSICHTMASSTAB = EINTRAEGE[10];
...
const SYMBOLMASSE = EINTRAEGE[11];
...
// er wird als einziger direkt ueber `EINTRAEGE[12]` geprueft; die bisherige Reihe rueckt geschlossen
...
const SAMMELPRODUKTE = EINTRAEGE[12];
...
const SPANNSYMBOLE = EINTRAEGE[13];
...
// es wird als einziges direkt ueber `EINTRAEGE[14]` geprueft; die bisherige Reihe rueckt geschlossen
...
const SAMMELMERKMALE = EINTRAEGE[14];
...
const EINLEGEMENGE = EINTRAEGE[15];
...
// `EINTRAEGE[16]` geprueft; die bisherige Reihe rueckt geschlossen um eins
...
const ZEICHNUNGSSYNC = EINTRAEGE[16];
...
// ueber `EINTRAEGE[17]` geprueft; die bisherige Reihe ist dafuer geschlossen um eins nach
...
const EDITIERMODUS = EINTRAEGE[17];
...
// direkt ueber `EINTRAEGE[18]` geprueft; die bisherige Reihe rueckt geschlossen um eins
...
const AUSGLEICHSMENGE = EINTRAEGE[18];
...
const MASSORIENTIERUNG = EINTRAEGE[19];
...
// als einzige direkt ueber `EINTRAEGE[20]` geprueft. Aussagewahr heisst hier: geliefert ist die
...
const AUSGLEICHSPUNKTE = EINTRAEGE[20];
...
const KATALOGMODUL = EINTRAEGE[21];
...
const PDFRUECKKEHR = EINTRAEGE[22];
...
const ACHSTREFFER = EINTRAEGE[23];
...
const HANDBUCHREGELN = EINTRAEGE[25];
...
// einziges direkt ueber `EINTRAEGE[26]` geprueft; die bisherige Reihe rueckt geschlossen um eins
...
// einzige direkt ueber `EINTRAEGE[27]` geprueft. Aussagewahr heisst hier: geliefert ist
...
// einziger direkt ueber `EINTRAEGE[28]` geprueft; die bisherige Reihe rueckt geschlossen um eins
...
// einziger direkt ueber `EINTRAEGE[30]` geprueft; die bisherige Reihe rueckt geschlossen um
...
// `EINTRAEGE[31]`, weil die Randregel der Spannachsen und die einheitliche Bezeichnung der
...
const EINBAUHOEHE = EINTRAEGE[31];
...
const EINBAULAGEN = EINTRAEGE[33];
...
const ZEICHNUNGSPDF = EINTRAEGE[34];
...
const VORSCHAUHOEHE = EINTRAEGE[35];
...
const STANDARDLAENGE = EINTRAEGE[36];
...
const KOPIERSCHUTZ = EINTRAEGE[37];
...
// zu drehen.
const VORHER = EINTRAEGE.slice(56);
const NEU = EINTRAEGE.slice(62);
const AKTUELL = EINTRAEGE.slice(67);
const AELTER = EINTRAEGE.slice(77);
...
const SPANNPLATTE = EINTRAEGE[39];
...
const WANDAUSWAHL = EINTRAEGE[40];
...
const VORRATSSATZ = EINTRAEGE[44];
...
const GESCHOSSMENGE = EINTRAEGE[46];
...
const TEILAUSWAHL = EINTRAEGE[47];
...
// unveraendert, es entsteht kein Feld und kein Versionssprung.
const IMPORTDIALOG = EINTRAEGE[48];
