...
// mit [A-3]/#97) sind der neueste Eintrag — er wird als einziger direkt ueber `EINTRAEGE[0]`
...
const ANSICHTMASSTAB = EINTRAEGE[0];
...
// sie werden als einzige direkt ueber `EINTRAEGE[1]` geprueft; die bisherige Reihe rueckt
...
// er wird als einziger direkt ueber `EINTRAEGE[2]` geprueft; die bisherige Reihe rueckt geschlossen
...
const SAMMELPRODUKTE = EINTRAEGE[2];
...
// sie werden als einzige direkt ueber `EINTRAEGE[3]` geprueft; die bisherige Reihe rueckt
// geschlossen um eins nach hinten. Aussagewahr heisst hier: geliefert ist AUSSCHLIESSLICH, dass
...
const SPANNSYMBOLE = EINTRAEGE[3];
...
// es wird als einziges direkt ueber `EINTRAEGE[4]` geprueft; die bisherige Reihe rueckt geschlossen
...
const SAMMELMERKMALE = EINTRAEGE[4];
...
const EINLEGEMENGE = EINTRAEGE[5];
...
// `EINTRAEGE[6]` geprueft; die bisherige Reihe rueckt geschlossen um eins
...
const ZEICHNUNGSSYNC = EINTRAEGE[6];
...
// ueber `EINTRAEGE[7]` geprueft; die bisherige Reihe ist dafuer geschlossen um eins nach
...
const EDITIERMODUS = EINTRAEGE[7];
...
// direkt ueber `EINTRAEGE[8]` geprueft; die bisherige Reihe rueckt geschlossen um eins
...
const AUSGLEICHSMENGE = EINTRAEGE[8];
...
const MASSORIENTIERUNG = EINTRAEGE[9];
...
// als einzige direkt ueber `EINTRAEGE[10]` geprueft. Aussagewahr heisst hier: geliefert ist die
...
const AUSGLEICHSPUNKTE = EINTRAEGE[10];
...
const KATALOGMODUL = EINTRAEGE[11];
...
const PDFRUECKKEHR = EINTRAEGE[12];
...
const ACHSTREFFER = EINTRAEGE[13];
...
const HANDBUCHREGELN = EINTRAEGE[15];
...
// einziges direkt ueber `EINTRAEGE[16]` geprueft; die bisherige Reihe rueckt geschlossen um eins
...
// einzige direkt ueber `EINTRAEGE[17]` geprueft. Aussagewahr heisst hier: geliefert ist
...
// einziger direkt ueber `EINTRAEGE[20]` geprueft; die bisherige Reihe rueckt geschlossen um
...
// `EINTRAEGE[21]`, weil die Randregel der Spannachsen und die einheitliche Bezeichnung der
...
const EINBAUHOEHE = EINTRAEGE[21];
...
const EINBAULAGEN = EINTRAEGE[23];
...
const ZEICHNUNGSPDF = EINTRAEGE[24];
...
const VORSCHAUHOEHE = EINTRAEGE[25];
...
const STANDARDLAENGE = EINTRAEGE[26];
...
const KOPIERSCHUTZ = EINTRAEGE[27];
...
// zu drehen.
const VORHER = EINTRAEGE.slice(46);
const NEU = EINTRAEGE.slice(52);
const AKTUELL = EINTRAEGE.slice(57);
const AELTER = EINTRAEGE.slice(67);
...
const SPANNPLATTE = EINTRAEGE[29];
...
const WANDAUSWAHL = EINTRAEGE[30];
...
const VORRATSSATZ = EINTRAEGE[34];
...
const GESCHOSSMENGE = EINTRAEGE[36];
...
const TEILAUSWAHL = EINTRAEGE[37];
...
// unveraendert, es entsteht kein Feld und kein Versionssprung.
const IMPORTDIALOG = EINTRAEGE[38];
