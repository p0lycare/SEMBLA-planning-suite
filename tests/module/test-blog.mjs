...
// Die NAMENSFOLGE DER WANDBLAETTER in der Zeichnungs-PDF (#107) ist der neueste Eintrag —
...
const PDFNAMENSFOLGE107 = EINTRAEGE[0];
ok("[#107] die Namensfolge der Wandblaetter ist der neueste Eintrag",
...
// als einziger direkt ueber `EINTRAEGE[1]` geprueft; die bisherige Reihe rueckt geschlossen
...
const STDKATALOG113 = EINTRAEGE[1];
...
// Eintrag — er wird als einziger direkt ueber `EINTRAEGE[2]` geprueft; die bisherige Reihe
...
const BESCHBLOCK113 = EINTRAEGE[2];
...
// zweitneueste Eintrag — er wird als einziger direkt ueber `EINTRAEGE[4]` geprueft; die bisherige
...
const BLECHSTOSS91 = EINTRAEGE[4];
...
// viertneueste Eintrag — er wird als einziger direkt ueber `EINTRAEGE[6]` geprueft; die
...
// `EINTRAEGE[7]` geprueft; die bisherige Reihe rueckt geschlossen um eins nach hinten.
...
const DECKENZEIGEN = EINTRAEGE[7];
...
// aus #94) ist der neueste Eintrag — er wird als einziger direkt ueber `EINTRAEGE[8]` geprueft;
...
// chg-20260907-20) ist der neueste Eintrag — er wird als einziger direkt ueber `EINTRAEGE[9]`
...
const SAMMELMEHRFACH = EINTRAEGE[9];
...
// einziger direkt ueber `EINTRAEGE[10]` geprueft; die bisherige Reihe rueckt geschlossen um eins
...
const OHNE_SCHEIBE = EINTRAEGE[10];
...
// mit [A-3]/#97) sind der neueste Eintrag — er wird als einziger direkt ueber `EINTRAEGE[11]`
...
const ANSICHTMASSTAB = EINTRAEGE[11];
...
// sie werden als einzige direkt ueber `EINTRAEGE[12]` geprueft; die bisherige Reihe rueckt
...
const SYMBOLMASSE = EINTRAEGE[12];
...
// er wird als einziger direkt ueber `EINTRAEGE[13]` geprueft; die bisherige Reihe rueckt geschlossen
// um eins nach hinten. Aussagewahr heisst hier: geliefert ist AUSSCHLIESSLICH, dass die
...
const SAMMELPRODUKTE = EINTRAEGE[13];
...
const SPANNSYMBOLE = EINTRAEGE[14];
...
// es wird als einziges direkt ueber `EINTRAEGE[15]` geprueft; die bisherige Reihe rueckt geschlossen
...
const SAMMELMERKMALE = EINTRAEGE[15];
...
const EINLEGEMENGE = EINTRAEGE[16];
...
// `EINTRAEGE[17]` geprueft; die bisherige Reihe rueckt geschlossen um eins
...
const ZEICHNUNGSSYNC = EINTRAEGE[17];
...
// ueber `EINTRAEGE[18]` geprueft; die bisherige Reihe ist dafuer geschlossen um eins nach
...
const EDITIERMODUS = EINTRAEGE[18];
...
// direkt ueber `EINTRAEGE[19]` geprueft; die bisherige Reihe rueckt geschlossen um eins
...
const AUSGLEICHSMENGE = EINTRAEGE[19];
...
const MASSORIENTIERUNG = EINTRAEGE[20];
...
// als einzige direkt ueber `EINTRAEGE[21]` geprueft. Aussagewahr heisst hier: geliefert ist die
...
const AUSGLEICHSPUNKTE = EINTRAEGE[21];
...
const KATALOGMODUL = EINTRAEGE[22];
...
const PDFRUECKKEHR = EINTRAEGE[23];
...
const ACHSTREFFER = EINTRAEGE[24];
...
const HANDBUCHREGELN = EINTRAEGE[26];
...
// einziges direkt ueber `EINTRAEGE[27]` geprueft; die bisherige Reihe rueckt geschlossen um eins
...
// einzige direkt ueber `EINTRAEGE[28]` geprueft. Aussagewahr heisst hier: geliefert ist
...
// einziger direkt ueber `EINTRAEGE[29]` geprueft; die bisherige Reihe rueckt geschlossen um eins
...
// einziger direkt ueber `EINTRAEGE[31]` geprueft; die bisherige Reihe rueckt geschlossen um
...
// `EINTRAEGE[32]`, weil die Randregel der Spannachsen und die einheitliche Bezeichnung der
...
const EINBAUHOEHE = EINTRAEGE[32];
...
const EINBAULAGEN = EINTRAEGE[34];
...
const ZEICHNUNGSPDF = EINTRAEGE[35];
...
const VORSCHAUHOEHE = EINTRAEGE[36];
...
const STANDARDLAENGE = EINTRAEGE[37];
...
const KOPIERSCHUTZ = EINTRAEGE[38];
...
// zu drehen.
const VORHER = EINTRAEGE.slice(57);
const NEU = EINTRAEGE.slice(63);
const AKTUELL = EINTRAEGE.slice(68);
const AELTER = EINTRAEGE.slice(78);
...
const SPANNPLATTE = EINTRAEGE[40];
...
const WANDAUSWAHL = EINTRAEGE[41];
...
const VORRATSSATZ = EINTRAEGE[45];
...
const GESCHOSSMENGE = EINTRAEGE[47];
...
const TEILAUSWAHL = EINTRAEGE[48];
...
// unveraendert, es entsteht kein Feld und kein Versionssprung.
const IMPORTDIALOG = EINTRAEGE[49];
