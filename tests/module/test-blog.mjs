...
// Der NEUESTE Eintrag ist die Stuecklistenposition des Ausgleichsblechs (#96) — sie wird als
...
const AUSGLEICHSMENGE = EINTRAEGE[0];
ok("[#96] die Stuecklistenposition des Ausgleichsblechs ist der neueste Eintrag",
...
const MASSORIENTIERUNG = EINTRAEGE[1];
...
// als einzige direkt ueber `EINTRAEGE[2]` geprueft. Aussagewahr heisst hier: geliefert ist die
...
const AUSGLEICHSPUNKTE = EINTRAEGE[2];
...
const KATALOGMODUL = EINTRAEGE[3];
...
const PDFRUECKKEHR = EINTRAEGE[4];
...
const HANDBUCHREGELN = EINTRAEGE[7];
...
// einziges direkt ueber `EINTRAEGE[8]` geprueft; die bisherige Reihe rueckt geschlossen um eins
...
// einzige direkt ueber `EINTRAEGE[9]` geprueft. Aussagewahr heisst hier: geliefert ist
...
// `EINTRAEGE[13]`, weil die Randregel der Spannachsen und die einheitliche Bezeichnung der
...
const EINBAUHOEHE = EINTRAEGE[13];
...
const EINBAULAGEN = EINTRAEGE[15];
...
const ZEICHNUNGSPDF = EINTRAEGE[16];
...
const VORSCHAUHOEHE = EINTRAEGE[17];
...
const STANDARDLAENGE = EINTRAEGE[18];
...
const KOPIERSCHUTZ = EINTRAEGE[19];
...
// zu drehen.
const VORHER = EINTRAEGE.slice(38);
const NEU = EINTRAEGE.slice(44);
const AKTUELL = EINTRAEGE.slice(49);
const AELTER = EINTRAEGE.slice(59);
...
const SPANNPLATTE = EINTRAEGE[21];
...
const WANDAUSWAHL = EINTRAEGE[22];
...
const VORRATSSATZ = EINTRAEGE[26];
...
const GESCHOSSMENGE = EINTRAEGE[28];
...
const TEILAUSWAHL = EINTRAEGE[29];
...
// unveraendert, es entsteht kein Feld und kein Versionssprung.
const IMPORTDIALOG = EINTRAEGE[30];
