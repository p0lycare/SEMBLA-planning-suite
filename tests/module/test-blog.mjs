...
// Der NEUESTE Eintrag ist die browserhohe Blattvorschau (#99) — er wird als einziger direkt
// ueber `EINTRAEGE[0]` geprueft; die bisherige Reihe rueckt geschlossen um eins nach hinten.
// Aussagewahr heisst hier: versprochen wird GENAU die Bildschirmhoehe des Vorschaubereichs in
// Modul 7 — in beiden Zustaenden (mit Zeichnung und ohne aktives Wandelement) — und dass das
// gedruckte Blatt einseitig bleibt; ausdruecklich NICHT versprochen werden ein anderes
// Blattformat, ein anderer Masstab, eine andere Blattgeometrie oder ein geaenderter
// Zeichnungsinhalt.
ok("[#99] die browserhohe Blattvorschau ist der neueste Eintrag",
  EINTRAEGE[0]?.id === "chg-20260904-12" && EINTRAEGE[0]?.issue === 99
...
ok("[#99] der Eintrag benennt Gegenstand und Nutzerergebnis",
  ...
  && /Browserfensters?/.test(EINTRAEGE[0]?.titel || "")
...
ok("[#99] die Testbitte fuehrt den Bedienweg samt beiden Zustaenden und dem Druck",
  ...
  && /Blattvorschau/.test(EINTRAEGE[0]?.testbitte || "")
  && /ohne aktives Wandelement/.test(EINTRAEGE[0]?.testbitte || "")
  && /einseitig/.test(EINTRAEGE[0]?.testbitte || ""));
...
const STANDARDLAENGE = EINTRAEGE[1];
...
const KOPIERSCHUTZ = EINTRAEGE[2];
...
// zu drehen.
const VORHER = EINTRAEGE.slice(21);
const NEU = EINTRAEGE.slice(27);
const AKTUELL = EINTRAEGE.slice(32);
const AELTER = EINTRAEGE.slice(42);
...
const GESCHOSSMENGE = EINTRAEGE[11];
...
const TEILAUSWAHL = EINTRAEGE[12];
...
// unveraendert, es entsteht kein Feld und kein Versionssprung.
const IMPORTDIALOG = EINTRAEGE[13];
