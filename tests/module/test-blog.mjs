...
// Die SCHLUESSELWEITE ALS KATALOGMASS (#97) ist der neueste Eintrag — er wird als einziger
...
const SCHLUESSELWEITE97 = EINTRAEGE[0];
...
// als einziger direkt ueber `EINTRAEGE[1]` geprueft; die bisherige Reihe rueckt geschlossen um
...
const NEURECHNUNG117 = EINTRAEGE[1];
...
const FEHLREFERENZ116 = EINTRAEGE[2];
...
// einziger direkt ueber `EINTRAEGE[3]` geprueft; die bisherige Reihe rueckt geschlossen um
...
const STANGENVORKOPPLUNG112 = EINTRAEGE[3];
...
// wird als einziger direkt ueber `EINTRAEGE[4]` geprueft; die bisherige Reihe rueckt
...
const ALTBESTAND114 = EINTRAEGE[4];
...
// direkt ueber `EINTRAEGE[5]` geprueft; die bisherige Reihe rueckt geschlossen um eins nach
...
const EBENENNACHWEIS94 = EINTRAEGE[5];
...
// einziger direkt ueber `EINTRAEGE[6]` geprueft; die bisherige Reihe rueckt geschlossen um eins
...
const MUTTERNHOEHE97 = EINTRAEGE[6];
...
const BAUGRUPPENANSICHT94 = EINTRAEGE[7];
...
// als einziger direkt ueber `EINTRAEGE[8]` geprueft; die bisherige Reihe rueckt geschlossen um
...
const EINKAUFSLISTE113 = EINTRAEGE[8];
...
// einziger direkt ueber `EINTRAEGE[9]` geprueft; die bisherige Reihe rueckt geschlossen um
...
const PLATTENDICKE97 = EINTRAEGE[9];
...
// Einbauhoehe der Kopplungsmutter (chg-20260909-08) und die Schluesselweite als Katalogmass
// (chg-20260909-14). Je Paket GENAU EIN Eintrag — und keiner mehr, damit dieselbe Aenderung
// nicht zweimal in der Liste steht.
...
// er wird als einziger direkt ueber `EINTRAEGE[10]` geprueft; die bisherige Reihe rueckt
// geschlossen um eins nach hinten. Aussagewahr heisst hier: geliefert ist AUSSCHLIESSLICH,
...
const PDFNAMENSFOLGE107 = EINTRAEGE[10];
...
// als einziger direkt ueber `EINTRAEGE[11]` geprueft; die bisherige Reihe rueckt geschlossen
...
const STDKATALOG113 = EINTRAEGE[11];
...
// Eintrag — er wird als einziger direkt ueber `EINTRAEGE[12]` geprueft; die bisherige Reihe
...
const BESCHBLOCK113 = EINTRAEGE[12];
...
const VARIANTE108 = EINTRAEGE[13];
...
// zweitneueste Eintrag — er wird als einziger direkt ueber `EINTRAEGE[14]` geprueft; die bisherige
...
const BLECHSTOSS91 = EINTRAEGE[14];
...
const BESCHAFFUNG = EINTRAEGE[15];
...
// viertneueste Eintrag — er wird als einziger direkt ueber `EINTRAEGE[16]` geprueft; die
...
const STANGENVORN = EINTRAEGE[16];
...
// `EINTRAEGE[17]` geprueft; die bisherige Reihe rueckt geschlossen um eins nach hinten.
...
const DECKENZEIGEN = EINTRAEGE[17];
...
// aus #94) ist der neueste Eintrag — er wird als einziger direkt ueber `EINTRAEGE[18]` geprueft;
...
// chg-20260907-20) ist der neueste Eintrag — er wird als einziger direkt ueber `EINTRAEGE[19]`
...
const SAMMELMEHRFACH = EINTRAEGE[19];
...
// einziger direkt ueber `EINTRAEGE[20]` geprueft; die bisherige Reihe rueckt geschlossen um eins
...
const OHNE_SCHEIBE = EINTRAEGE[20];
...
// mit [A-3]/#97) sind der neueste Eintrag — er wird als einziger direkt ueber `EINTRAEGE[21]`
...
const ANSICHTMASSTAB = EINTRAEGE[21];
...
// sie werden als einzige direkt ueber `EINTRAEGE[22]` geprueft; die bisherige Reihe rueckt
...
const SYMBOLMASSE = EINTRAEGE[22];
...
// er wird als einziger direkt ueber `EINTRAEGE[23]` geprueft; die bisherige Reihe rueckt geschlossen
// um eins nach hinten. Aussagewahr heisst hier: geliefert ist AUSSCHLIESSLICH, dass die
...
const SAMMELPRODUKTE = EINTRAEGE[23];
...
const SPANNSYMBOLE = EINTRAEGE[24];
...
// es wird als einziges direkt ueber `EINTRAEGE[25]` geprueft; die bisherige Reihe rueckt geschlossen
...
const SAMMELMERKMALE = EINTRAEGE[25];
...
const EINLEGEMENGE = EINTRAEGE[26];
...
// `EINTRAEGE[27]` geprueft; die bisherige Reihe rueckt geschlossen um eins
...
const ZEICHNUNGSSYNC = EINTRAEGE[27];
...
// ueber `EINTRAEGE[28]` geprueft; die bisherige Reihe ist dafuer geschlossen um eins nach
...
const EDITIERMODUS = EINTRAEGE[28];
...
// direkt ueber `EINTRAEGE[29]` geprueft; die bisherige Reihe rueckt geschlossen um eins
...
const AUSGLEICHSMENGE = EINTRAEGE[29];
...
// `EINTRAEGE[30]` geprueft; die bisherige Reihe rueckt geschlossen um eins nach hinten.
...
const MASSORIENTIERUNG = EINTRAEGE[30];
...
// als einzige direkt ueber `EINTRAEGE[31]` geprueft. Aussagewahr heisst hier: geliefert ist die
...
const AUSGLEICHSPUNKTE = EINTRAEGE[31];
...
const KATALOGMODUL = EINTRAEGE[32];
...
const PDFRUECKKEHR = EINTRAEGE[33];
...
const ACHSTREFFER = EINTRAEGE[34];
...
const HANDBUCHREGELN = EINTRAEGE[36];
...
// einziges direkt ueber `EINTRAEGE[37]` geprueft; die bisherige Reihe rueckt geschlossen um eins
...
// einzige direkt ueber `EINTRAEGE[38]` geprueft. Aussagewahr heisst hier: geliefert ist
...
// einziger direkt ueber `EINTRAEGE[39]` geprueft; die bisherige Reihe rueckt geschlossen um eins
...
// einziger direkt ueber `EINTRAEGE[41]` geprueft; die bisherige Reihe rueckt geschlossen um
...
// `EINTRAEGE[42]`, weil die Randregel der Spannachsen und die einheitliche Bezeichnung der
...
const EINBAUHOEHE = EINTRAEGE[42];
...
const EINBAULAGEN = EINTRAEGE[44];
...
const ZEICHNUNGSPDF = EINTRAEGE[45];
...
const VORSCHAUHOEHE = EINTRAEGE[46];
...
const STANDARDLAENGE = EINTRAEGE[47];
...
const KOPIERSCHUTZ = EINTRAEGE[48];
...
// zu drehen.
const VORHER = EINTRAEGE.slice(67);
const NEU = EINTRAEGE.slice(73);
const AKTUELL = EINTRAEGE.slice(78);
const AELTER = EINTRAEGE.slice(88);
...
const SPANNPLATTE = EINTRAEGE[50];
...
const WANDAUSWAHL = EINTRAEGE[51];
...
const VORRATSSATZ = EINTRAEGE[55];
...
const GESCHOSSMENGE = EINTRAEGE[57];
...
const TEILAUSWAHL = EINTRAEGE[58];
...
// unveraendert, es entsteht kein Feld und kein Versionssprung.
const IMPORTDIALOG = EINTRAEGE[59];
