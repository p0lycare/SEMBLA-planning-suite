...
ok("die textfreien Modulstarts 1 bis 4 (Issue 72) sind der neueste Eintrag",
  EINTRAEGE[0]?.id === "chg-20260813-04" && EINTRAEGE[0]?.issue === 72);
ok("der Intro-Eintrag benennt Modulbereich und unveränderte Funktionen aussagewahr",
  /Module 1 bis 4/.test(EINTRAEGE[0]?.titel || "")
  && /Kopfleiste/.test(EINTRAEGE[0]?.testbitte || "")
  && /unverändert/.test(EINTRAEGE[0]?.testbitte || ""));
ok("der Geschossplaner-Reiter 0,5 (Issue 43) folgt direkt danach",
  EINTRAEGE[1]?.id === "chg-20260813-03" && EINTRAEGE[1]?.issue === 43);
ok("der Reiter-Eintrag benennt Kopfleiste, aktives Geschoss und unveränderte Auswahl aussagewahr",
  /Reiter 0,5/.test(EINTRAEGE[1]?.titel || "")
  && /aktive Geschoss/.test(EINTRAEGE[1]?.testbitte || "")
  && /Auswahl/.test(EINTRAEGE[1]?.testbitte || ""));
ok("die Lageplan-Nummernblasen (Issue 73) bleiben als dritter aktueller Eintrag erhalten",
  EINTRAEGE[2]?.id === "chg-20260813-02" && EINTRAEGE[2]?.issue === 73);
ok("der Marker-Eintrag benennt Außenblase und entfallenen Vollständigkeitsblock aussagewahr",
  /Nummernblase/.test(EINTRAEGE[2]?.testbitte || "")
  && /Vollständigkeit/.test(EINTRAEGE[2]?.testbitte || "")
  && /entfallen/.test(EINTRAEGE[2]?.testbitte || ""));
ok("der hierarchische Export (Issue 67) bleibt als vierter aktueller Eintrag erhalten",
  EINTRAEGE[3]?.id === "chg-20260813-01" && EINTRAEGE[3]?.issue === 67);
ok("der Export-Eintrag benennt den Wegfall des Planbild-Transports aussagewahr",
  /Planbilder/.test(EINTRAEGE[3]?.testbitte || "") && /entfallen/.test(EINTRAEGE[3]?.testbitte || ""));
ok("die kollisionsfreien Maßzahlen (Issue 59) bleiben als fünfter aktueller Eintrag erhalten",
  EINTRAEGE[4]?.id === "chg-20260812-10" && EINTRAEGE[4]?.issue === 59);
ok("die textfreie Eingabespalte in Modul 1 (Issue 69, zweite Kürzung) bleibt als sechster aktueller Eintrag erhalten",
  EINTRAEGE[5]?.id === "chg-20260812-09" && EINTRAEGE[5]?.issue === 69);
ok("die fokussierte Projektanlage (Issue 68) bleibt als siebter aktueller Eintrag erhalten",
  EINTRAEGE[6]?.id === "chg-20260812-08" && EINTRAEGE[6]?.issue === 68);
ok("die Wandbezeichnung in der Stückliste (Issue 70) bleibt als achter aktueller Eintrag erhalten",
  EINTRAEGE[7]?.id === "chg-20260812-07" && EINTRAEGE[7]?.issue === 70);
ok("die kompakte Eingabespalte in Modul 1 (Issue 69, erste Kürzung) bleibt als neunter aktueller Eintrag erhalten",
  EINTRAEGE[8]?.id === "chg-20260812-06" && EINTRAEGE[8]?.issue === 69);
ok("die aktive Wand im Geschosseditor (Issue 66) bleibt als zehnter aktueller Eintrag erhalten",
  EINTRAEGE[9]?.id === "chg-20260812-05" && EINTRAEGE[9]?.issue === 66);
ok("die Blattreduktion zu Issue 61 bleibt als elfter aktueller Eintrag erhalten",
  EINTRAEGE[10]?.id === "chg-20260812-04" && EINTRAEGE[10]?.issue === 61);
ok("der kompakte Lageplankopf zu Issue 59 bleibt als zwölfter aktueller Eintrag erhalten",
  EINTRAEGE[11]?.id === "chg-20260812-03" && EINTRAEGE[11]?.issue === 59);
ok("der Eintrag zu Issue 56 bleibt als dreizehnter aktueller Eintrag erhalten",
  EINTRAEGE[12]?.id === "chg-20260812-02" && EINTRAEGE[12]?.issue === 56);
