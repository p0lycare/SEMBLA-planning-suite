...
const FEHLREFERENZ116 = EINTRAEGE[0];
...
// einziger direkt ueber `EINTRAEGE[1]` geprueft; die bisherige Reihe rueckt geschlossen um
...
const STANGENVORKOPPLUNG112 = EINTRAEGE[1];
...
// wird als einziger direkt ueber `EINTRAEGE[2]` geprueft; die bisherige Reihe rueckt
...
const ALTBESTAND114 = EINTRAEGE[2];
...
const EBENENNACHWEIS94 = EINTRAEGE[3];
...
const MUTTERNHOEHE97 = EINTRAEGE[4];
...
const BAUGRUPPENANSICHT94 = EINTRAEGE[5];
...
// als einziger direkt ueber `EINTRAEGE[6]` geprueft; die bisherige Reihe rueckt geschlossen um
...
const EINKAUFSLISTE113 = EINTRAEGE[6];
...
// einziger direkt ueber `EINTRAEGE[7]` geprueft; die bisherige Reihe rueckt geschlossen um
...
const PLATTENDICKE97 = EINTRAEGE[7];
...
// er wird als einziger direkt ueber `EINTRAEGE[8]` geprueft; die bisherige Reihe rueckt
// geschlossen um eins nach hinten. Aussagewahr heisst hier: geliefert ist AUSSCHLIESSLICH,
...
const PDFNAMENSFOLGE107 = EINTRAEGE[8];
...
// als einziger direkt ueber `EINTRAEGE[9]` geprueft; die bisherige Reihe rueckt geschlossen
...
const STDKATALOG113 = EINTRAEGE[9];
...
// Eintrag — er wird als einziger direkt ueber `EINTRAEGE[10]` geprueft; die bisherige Reihe
...
const BESCHBLOCK113 = EINTRAEGE[10];
...
const VARIANTE108 = EINTRAEGE[11];
...
// zweitneueste Eintrag — er wird als einziger direkt ueber `EINTRAEGE[12]` geprueft; die bisherige
...
const BLECHSTOSS91 = EINTRAEGE[12];
...
const BESCHAFFUNG = EINTRAEGE[13];
...
// viertneueste Eintrag — er wird als einziger direkt ueber `EINTRAEGE[14]` geprueft; die
...
const STANGENVORN = EINTRAEGE[14];
...
const DECKENZEIGEN = EINTRAEGE[15];
...
// aus #94) ist der neueste Eintrag — er wird als einziger direkt ueber `EINTRAEGE[16]` geprueft;
...
// chg-20260907-20) ist der neueste Eintrag — er wird als einziger direkt ueber `EINTRAEGE[17]`
...
const SAMMELMEHRFACH = EINTRAEGE[17];
...
// einziger direkt ueber `EINTRAEGE[18]` geprueft; die bisherige Reihe rueckt geschlossen um eins
...
const OHNE_SCHEIBE = EINTRAEGE[18];
...
// mit [A-3]/#97) sind der neueste Eintrag — er wird als einziger direkt ueber `EINTRAEGE[19]`
...
const ANSICHTMASSTAB = EINTRAEGE[19];
...
const SYMBOLMASSE = EINTRAEGE[20];
...
// er wird als einziger direkt ueber `EINTRAEGE[21]` geprueft; die bisherige Reihe rueckt geschlossen
// um eins nach hinten. Aussagewahr heisst hier: geliefert ist AUSSCHLIESSLICH, dass die
...
const SAMMELPRODUKTE = EINTRAEGE[21];
...
const SPANNSYMBOLE = EINTRAEGE[22];
...
// es wird als einziges direkt ueber `EINTRAEGE[23]` geprueft; die bisherige Reihe rueckt geschlossen
...
const SAMMELMERKMALE = EINTRAEGE[23];
...
const EINLEGEMENGE = EINTRAEGE[24];
...
// `EINTRAEGE[25]` geprueft; die bisherige Reihe rueckt geschlossen um eins
...
const ZEICHNUNGSSYNC = EINTRAEGE[25];
...
// ueber `EINTRAEGE[26]` geprueft; die bisherige Reihe ist dafuer geschlossen um eins nach
...
const EDITIERMODUS = EINTRAEGE[26];
...
// direkt ueber `EINTRAEGE[27]` geprueft; die bisherige Reihe rueckt geschlossen um eins
...
const AUSGLEICHSMENGE = EINTRAEGE[27];
...
// `EINTRAEGE[28]` geprueft; die bisherige Reihe rueckt geschlossen um eins nach hinten.
...
const MASSORIENTIERUNG = EINTRAEGE[28];
...
// als einzige direkt ueber `EINTRAEGE[29]` geprueft. Aussagewahr heisst hier: geliefert ist die
...
const AUSGLEICHSPUNKTE = EINTRAEGE[29];
...
const KATALOGMODUL = EINTRAEGE[30];
...
const PDFRUECKKEHR = EINTRAEGE[31];
...
const ACHSTREFFER = EINTRAEGE[32];
...
const HANDBUCHREGELN = EINTRAEGE[34];
...
// einziges direkt ueber `EINTRAEGE[35]` geprueft; die bisherige Reihe rueckt geschlossen um eins
...
// einzige direkt ueber `EINTRAEGE[36]` geprueft. Aussagewahr heisst hier: geliefert ist
...
// einziger direkt ueber `EINTRAEGE[37]` geprueft; die bisherige Reihe rueckt geschlossen um eins
...
// einziger direkt ueber `EINTRAEGE[39]` geprueft; die bisherige Reihe rueckt geschlossen um
...
// `EINTRAEGE[40]`, weil die Randregel der Spannachsen und die einheitliche Bezeichnung der
...
const EINBAUHOEHE = EINTRAEGE[40];
...
const EINBAULAGEN = EINTRAEGE[42];
...
const ZEICHNUNGSPDF = EINTRAEGE[43];
...
const VORSCHAUHOEHE = EINTRAEGE[44];
...
const STANDARDLAENGE = EINTRAEGE[45];
...
const KOPIERSCHUTZ = EINTRAEGE[46];
...
// zu drehen.
const VORHER = EINTRAEGE.slice(65);
const NEU = EINTRAEGE.slice(71);
const AKTUELL = EINTRAEGE.slice(76);
const AELTER = EINTRAEGE.slice(86);
...
const SPANNPLATTE = EINTRAEGE[48];
...
const WANDAUSWAHL = EINTRAEGE[49];
...
const VORRATSSATZ = EINTRAEGE[53];
...
const GESCHOSSMENGE = EINTRAEGE[55];
...
const TEILAUSWAHL = EINTRAEGE[56];
...
// unveraendert, es entsteht kein Feld und kein Versionssprung.
const IMPORTDIALOG = EINTRAEGE[57];
