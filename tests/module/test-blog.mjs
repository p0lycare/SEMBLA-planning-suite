...
// Die vereinfachten Seitenansicht-Symbole der Spannkomponenten (#110) sind der neueste Eintrag —
...
const SPANNSYMBOLE = EINTRAEGE[0];
ok("[#110] die Symbole der Spannkomponenten sind der neueste Eintrag",
...
// Das gemeinsame Bearbeiten aller neun allgemeinen Wandmerkmale (#111) ist der neueste Eintrag —
...
const SAMMELMERKMALE = EINTRAEGE[1];
ok("[#111] das gemeinsame Bearbeiten der Wandmerkmale ist der neueste Eintrag",
...
ok("genau ein Eintrag fuer Issue 111", EINTRAEGE.filter(e => e.issue === 111).length === 1);
...
const EINLEGEMENGE = EINTRAEGE[2];
...
// `EINTRAEGE[3]` geprueft; die bisherige Reihe rueckt geschlossen um eins
...
const ZEICHNUNGSSYNC = EINTRAEGE[3];
...
// ueber `EINTRAEGE[4]` geprueft; die bisherige Reihe ist dafuer geschlossen um eins nach
...
const EDITIERMODUS = EINTRAEGE[4];
...
// direkt ueber `EINTRAEGE[5]` geprueft; die bisherige Reihe rueckt geschlossen um eins
...
const AUSGLEICHSMENGE = EINTRAEGE[5];
...
const MASSORIENTIERUNG = EINTRAEGE[6];
...
// als einzige direkt ueber `EINTRAEGE[7]` geprueft. Aussagewahr heisst hier: geliefert ist die
...
const AUSGLEICHSPUNKTE = EINTRAEGE[7];
...
const KATALOGMODUL = EINTRAEGE[8];
...
const PDFRUECKKEHR = EINTRAEGE[9];
...
const ACHSTREFFER = EINTRAEGE[10];
...
const HANDBUCHREGELN = EINTRAEGE[12];
...
// einziges direkt ueber `EINTRAEGE[13]` geprueft; die bisherige Reihe rueckt geschlossen um eins
...
// einzige direkt ueber `EINTRAEGE[14]` geprueft. Aussagewahr heisst hier: geliefert ist
...
// einziger direkt ueber `EINTRAEGE[17]` geprueft; die bisherige Reihe rueckt geschlossen um
...
// `EINTRAEGE[18]`, weil die Randregel der Spannachsen und die einheitliche Bezeichnung der
...
const EINBAUHOEHE = EINTRAEGE[18];
...
const EINBAULAGEN = EINTRAEGE[20];
...
const ZEICHNUNGSPDF = EINTRAEGE[21];
...
const VORSCHAUHOEHE = EINTRAEGE[22];
...
const STANDARDLAENGE = EINTRAEGE[23];
...
const KOPIERSCHUTZ = EINTRAEGE[24];
...
// zu drehen.
const VORHER = EINTRAEGE.slice(43);
const NEU = EINTRAEGE.slice(49);
const AKTUELL = EINTRAEGE.slice(54);
const AELTER = EINTRAEGE.slice(64);
...
const SPANNPLATTE = EINTRAEGE[26];
...
const WANDAUSWAHL = EINTRAEGE[27];
...
const VORRATSSATZ = EINTRAEGE[31];
...
const GESCHOSSMENGE = EINTRAEGE[33];
...
const TEILAUSWAHL = EINTRAEGE[34];
...
// unveraendert, es entsteht kein Feld und kein Versionssprung.
const IMPORTDIALOG = EINTRAEGE[35];
