...
const STANGENVORKOPPLUNG112 = EINTRAEGE[0];
...
// wird als einziger direkt ueber `EINTRAEGE[1]` geprueft; die bisherige Reihe rueckt
...
const ALTBESTAND114 = EINTRAEGE[1];
...
const EBENENNACHWEIS94 = EINTRAEGE[2];
...
// einziger direkt ueber `EINTRAEGE[3]` geprueft; die bisherige Reihe rueckt geschlossen um eins
...
const MUTTERNHOEHE97 = EINTRAEGE[3];
...
const BAUGRUPPENANSICHT94 = EINTRAEGE[4];
...
const EINKAUFSLISTE113 = EINTRAEGE[5];
...
const PLATTENDICKE97 = EINTRAEGE[6];
...
const PDFNAMENSFOLGE107 = EINTRAEGE[7];
...
// als einziger direkt ueber `EINTRAEGE[8]` geprueft; die bisherige Reihe rueckt geschlossen
...
const STDKATALOG113 = EINTRAEGE[8];
...
// Eintrag — er wird als einziger direkt ueber `EINTRAEGE[9]` geprueft; die bisherige Reihe
...
const BESCHBLOCK113 = EINTRAEGE[9];
...
const VARIANTE108 = EINTRAEGE[10];
...
// zweitneueste Eintrag — er wird als einziger direkt ueber `EINTRAEGE[11]` geprueft; die bisherige
...
const BLECHSTOSS91 = EINTRAEGE[11];
...
const BESCHAFFUNG = EINTRAEGE[12];
...
// viertneueste Eintrag — er wird als einziger direkt ueber `EINTRAEGE[13]` geprueft; die
...
const STANGENVORN = EINTRAEGE[13];
...
const DECKENZEIGEN = EINTRAEGE[14];
...
// aus #94) ist der neueste Eintrag — er wird als einziger direkt ueber `EINTRAEGE[15]` geprueft;
...
// chg-20260907-20) ist der neueste Eintrag — er wird als einziger direkt ueber `EINTRAEGE[16]`
...
const SAMMELMEHRFACH = EINTRAEGE[16];
...
// einziger direkt ueber `EINTRAEGE[17]` geprueft; die bisherige Reihe rueckt geschlossen um eins
...
const OHNE_SCHEIBE = EINTRAEGE[17];
...
// mit [A-3]/#97) sind der neueste Eintrag — er wird als einziger direkt ueber `EINTRAEGE[18]`
...
const ANSICHTMASSTAB = EINTRAEGE[18];
...
const SYMBOLMASSE = EINTRAEGE[19];
...
// er wird als einziger direkt ueber `EINTRAEGE[20]` geprueft; die bisherige Reihe rueckt geschlossen
// um eins nach hinten. Aussagewahr heisst hier: geliefert ist AUSSCHLIESSLICH, dass die
...
const SAMMELPRODUKTE = EINTRAEGE[20];
...
const SPANNSYMBOLE = EINTRAEGE[21];
...
// es wird als einziges direkt ueber `EINTRAEGE[22]` geprueft; die bisherige Reihe rueckt geschlossen
...
const SAMMELMERKMALE = EINTRAEGE[22];
...
const EINLEGEMENGE = EINTRAEGE[23];
...
// `EINTRAEGE[24]` geprueft; die bisherige Reihe rueckt geschlossen um eins
...
const ZEICHNUNGSSYNC = EINTRAEGE[24];
...
// ueber `EINTRAEGE[25]` geprueft; die bisherige Reihe ist dafuer geschlossen um eins nach
...
const EDITIERMODUS = EINTRAEGE[25];
...
// direkt ueber `EINTRAEGE[26]` geprueft; die bisherige Reihe rueckt geschlossen um eins
...
const AUSGLEICHSMENGE = EINTRAEGE[26];
...
// `EINTRAEGE[27]` geprueft; die bisherige Reihe rueckt geschlossen um eins nach hinten.
...
const MASSORIENTIERUNG = EINTRAEGE[27];
...
// als einzige direkt ueber `EINTRAEGE[28]` geprueft. Aussagewahr heisst hier: geliefert ist die
...
const AUSGLEICHSPUNKTE = EINTRAEGE[28];
...
const KATALOGMODUL = EINTRAEGE[29];
...
const PDFRUECKKEHR = EINTRAEGE[30];
...
const ACHSTREFFER = EINTRAEGE[31];
...
const HANDBUCHREGELN = EINTRAEGE[33];
...
// einziges direkt ueber `EINTRAEGE[34]` geprueft; die bisherige Reihe rueckt geschlossen um eins
...
// einzige direkt ueber `EINTRAEGE[35]` geprueft. Aussagewahr heisst hier: geliefert ist
...
// einziger direkt ueber `EINTRAEGE[36]` geprueft; die bisherige Reihe rueckt geschlossen um eins
...
// einziger direkt ueber `EINTRAEGE[38]` geprueft; die bisherige Reihe rueckt geschlossen um
...
// `EINTRAEGE[39]`, weil die Randregel der Spannachsen und die einheitliche Bezeichnung der
...
const EINBAUHOEHE = EINTRAEGE[39];
...
const EINBAULAGEN = EINTRAEGE[41];
...
const ZEICHNUNGSPDF = EINTRAEGE[42];
...
const VORSCHAUHOEHE = EINTRAEGE[43];
...
const STANDARDLAENGE = EINTRAEGE[44];
...
const KOPIERSCHUTZ = EINTRAEGE[45];
...
// zu drehen.
const VORHER = EINTRAEGE.slice(64);
const NEU = EINTRAEGE.slice(70);
const AKTUELL = EINTRAEGE.slice(75);
const AELTER = EINTRAEGE.slice(85);
...
const SPANNPLATTE = EINTRAEGE[47];
...
const WANDAUSWAHL = EINTRAEGE[48];
...
const VORRATSSATZ = EINTRAEGE[52];
...
const GESCHOSSMENGE = EINTRAEGE[54];
...
const TEILAUSWAHL = EINTRAEGE[55];
...
// unveraendert, es entsteht kein Feld und kein Versionssprung.
const IMPORTDIALOG = EINTRAEGE[56];
