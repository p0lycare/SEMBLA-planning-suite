...
const ALTBESTAND114 = EINTRAEGE[0];
...
const EBENENNACHWEIS94 = EINTRAEGE[1];
...
// einziger direkt ueber `EINTRAEGE[2]` geprueft; die bisherige Reihe rueckt geschlossen um eins
...
const MUTTERNHOEHE97 = EINTRAEGE[2];
...
const BAUGRUPPENANSICHT94 = EINTRAEGE[3];
...
const EINKAUFSLISTE113 = EINTRAEGE[4];
...
const PLATTENDICKE97 = EINTRAEGE[5];
...
const PDFNAMENSFOLGE107 = EINTRAEGE[6];
...
// als einziger direkt ueber `EINTRAEGE[7]` geprueft; die bisherige Reihe rueckt geschlossen
...
const STDKATALOG113 = EINTRAEGE[7];
...
// Eintrag — er wird als einziger direkt ueber `EINTRAEGE[8]` geprueft; die bisherige Reihe
...
const BESCHBLOCK113 = EINTRAEGE[8];
...
// zweitneueste Eintrag — er wird als einziger direkt ueber `EINTRAEGE[10]` geprueft; die bisherige
...
const BLECHSTOSS91 = EINTRAEGE[10];
...
const BESCHAFFUNG = EINTRAEGE[11];
...
// viertneueste Eintrag — er wird als einziger direkt ueber `EINTRAEGE[12]` geprueft; die
...
const STANGENVORN = EINTRAEGE[12];
...
// `EINTRAEGE[13]` geprueft; die bisherige Reihe rueckt geschlossen um eins nach hinten.
...
const DECKENZEIGEN = EINTRAEGE[13];
...
// aus #94) ist der neueste Eintrag — er wird als einziger direkt ueber `EINTRAEGE[14]` geprueft;
...
// chg-20260907-20) ist der neueste Eintrag — er wird als einziger direkt ueber `EINTRAEGE[15]`
...
const SAMMELMEHRFACH = EINTRAEGE[15];
...
// einziger direkt ueber `EINTRAEGE[16]` geprueft; die bisherige Reihe rueckt geschlossen um eins
...
const OHNE_SCHEIBE = EINTRAEGE[16];
...
// mit [A-3]/#97) sind der neueste Eintrag — er wird als einziger direkt ueber `EINTRAEGE[17]`
...
const ANSICHTMASSTAB = EINTRAEGE[17];
...
const SYMBOLMASSE = EINTRAEGE[18];
...
// er wird als einziger direkt ueber `EINTRAEGE[19]` geprueft; die bisherige Reihe rueckt geschlossen
// um eins nach hinten. Aussagewahr heisst hier: geliefert ist AUSSCHLIESSLICH, dass die
...
const SAMMELPRODUKTE = EINTRAEGE[19];
...
const SPANNSYMBOLE = EINTRAEGE[20];
...
// es wird als einziges direkt ueber `EINTRAEGE[21]` geprueft; die bisherige Reihe rueckt geschlossen
...
const SAMMELMERKMALE = EINTRAEGE[21];
...
const EINLEGEMENGE = EINTRAEGE[22];
...
// `EINTRAEGE[23]` geprueft; die bisherige Reihe rueckt geschlossen um eins
...
const ZEICHNUNGSSYNC = EINTRAEGE[23];
...
// ueber `EINTRAEGE[24]` geprueft; die bisherige Reihe ist dafuer geschlossen um eins nach
...
const EDITIERMODUS = EINTRAEGE[24];
...
// direkt ueber `EINTRAEGE[25]` geprueft; die bisherige Reihe rueckt geschlossen um eins
...
const AUSGLEICHSMENGE = EINTRAEGE[25];
...
// `EINTRAEGE[26]` geprueft; die bisherige Reihe rueckt geschlossen um eins nach hinten.
...
const MASSORIENTIERUNG = EINTRAEGE[26];
...
// als einzige direkt ueber `EINTRAEGE[27]` geprueft. Aussagewahr heisst hier: geliefert ist die
...
const AUSGLEICHSPUNKTE = EINTRAEGE[27];
...
const KATALOGMODUL = EINTRAEGE[28];
...
const PDFRUECKKEHR = EINTRAEGE[29];
...
const ACHSTREFFER = EINTRAEGE[30];
...
const HANDBUCHREGELN = EINTRAEGE[32];
...
// einziges direkt ueber `EINTRAEGE[33]` geprueft; die bisherige Reihe rueckt geschlossen um eins
...
// einzige direkt ueber `EINTRAEGE[34]` geprueft. Aussagewahr heisst hier: geliefert ist
...
// einziger direkt ueber `EINTRAEGE[35]` geprueft; die bisherige Reihe rueckt geschlossen um eins
...
// einziger direkt ueber `EINTRAEGE[37]` geprueft; die bisherige Reihe rueckt geschlossen um
...
// `EINTRAEGE[38]`, weil die Randregel der Spannachsen und die einheitliche Bezeichnung der
...
const EINBAUHOEHE = EINTRAEGE[38];
...
const EINBAULAGEN = EINTRAEGE[40];
...
const ZEICHNUNGSPDF = EINTRAEGE[41];
...
const VORSCHAUHOEHE = EINTRAEGE[42];
...
const STANDARDLAENGE = EINTRAEGE[43];
...
const KOPIERSCHUTZ = EINTRAEGE[44];
...
// zu drehen.
const VORHER = EINTRAEGE.slice(63);
const NEU = EINTRAEGE.slice(69);
const AKTUELL = EINTRAEGE.slice(74);
const AELTER = EINTRAEGE.slice(84);
...
const SPANNPLATTE = EINTRAEGE[46];
...
const WANDAUSWAHL = EINTRAEGE[47];
...
const VORRATSSATZ = EINTRAEGE[51];
...
const GESCHOSSMENGE = EINTRAEGE[53];
...
const TEILAUSWAHL = EINTRAEGE[54];
...
// unveraendert, es entsteht kein Feld und kein Versionssprung.
const IMPORTDIALOG = EINTRAEGE[55];
