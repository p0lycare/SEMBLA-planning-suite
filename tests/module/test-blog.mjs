...
// er wird als einziger direkt ueber `EINTRAEGE[0]` geprueft; die bisherige Reihe rueckt geschlossen
...
const SAMMELPRODUKTE = EINTRAEGE[0];
...
const SPANNSYMBOLE = EINTRAEGE[1];
...
// es wird als einziges direkt ueber `EINTRAEGE[2]` geprueft; die bisherige Reihe rueckt geschlossen
...
const SAMMELMERKMALE = EINTRAEGE[2];
...
const EINLEGEMENGE = EINTRAEGE[3];
...
// `EINTRAEGE[4]` geprueft; die bisherige Reihe rueckt geschlossen um eins
...
const ZEICHNUNGSSYNC = EINTRAEGE[4];
...
// ueber `EINTRAEGE[5]` geprueft; die bisherige Reihe ist dafuer geschlossen um eins nach
...
const EDITIERMODUS = EINTRAEGE[5];
...
// direkt ueber `EINTRAEGE[6]` geprueft; die bisherige Reihe rueckt geschlossen um eins
...
const AUSGLEICHSMENGE = EINTRAEGE[6];
...
const MASSORIENTIERUNG = EINTRAEGE[7];
...
// als einzige direkt ueber `EINTRAEGE[8]` geprueft. Aussagewahr heisst hier: geliefert ist die
...
const AUSGLEICHSPUNKTE = EINTRAEGE[8];
...
const KATALOGMODUL = EINTRAEGE[9];
...
const PDFRUECKKEHR = EINTRAEGE[10];
...
const ACHSTREFFER = EINTRAEGE[11];
...
const HANDBUCHREGELN = EINTRAEGE[13];
...
// einziges direkt ueber `EINTRAEGE[14]` geprueft; die bisherige Reihe rueckt geschlossen um eins
...
// einzige direkt ueber `EINTRAEGE[15]` geprueft. Aussagewahr heisst hier: geliefert ist
...
// einziger direkt ueber `EINTRAEGE[18]` geprueft; die bisherige Reihe rueckt geschlossen um
...
// `EINTRAEGE[19]`, weil die Randregel der Spannachsen und die einheitliche Bezeichnung der
...
const EINBAUHOEHE = EINTRAEGE[19];
...
const EINBAULAGEN = EINTRAEGE[21];
...
const ZEICHNUNGSPDF = EINTRAEGE[22];
...
const VORSCHAUHOEHE = EINTRAEGE[23];
...
const STANDARDLAENGE = EINTRAEGE[24];
...
const KOPIERSCHUTZ = EINTRAEGE[25];
...
// zu drehen.
const VORHER = EINTRAEGE.slice(44);
const NEU = EINTRAEGE.slice(50);
const AKTUELL = EINTRAEGE.slice(55);
const AELTER = EINTRAEGE.slice(65);
...
const SPANNPLATTE = EINTRAEGE[27];
...
const WANDAUSWAHL = EINTRAEGE[28];
...
const VORRATSSATZ = EINTRAEGE[32];
...
const GESCHOSSMENGE = EINTRAEGE[34];
...
const TEILAUSWAHL = EINTRAEGE[35];
...
// unveraendert, es entsteht kein Feld und kein Versionssprung.
const IMPORTDIALOG = EINTRAEGE[36];
