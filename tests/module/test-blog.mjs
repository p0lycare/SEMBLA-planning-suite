...
const BESCHBLOCK113 = EINTRAEGE[0];
...
// zweitneueste Eintrag — er wird als einziger direkt ueber `EINTRAEGE[2]` geprueft; die bisherige
...
const BLECHSTOSS91 = EINTRAEGE[2];
...
// viertneueste Eintrag — er wird als einziger direkt ueber `EINTRAEGE[4]` geprueft; die
...
// `EINTRAEGE[5]` geprueft; die bisherige Reihe rueckt geschlossen um eins nach hinten.
...
const DECKENZEIGEN = EINTRAEGE[5];
...
// aus #94) ist der neueste Eintrag — er wird als einziger direkt ueber `EINTRAEGE[6]` geprueft;
...
// chg-20260907-20) ist der neueste Eintrag — er wird als einziger direkt ueber `EINTRAEGE[7]`
...
const SAMMELMEHRFACH = EINTRAEGE[7];
...
const OHNE_SCHEIBE = EINTRAEGE[8];
...
// mit [A-3]/#97) sind der neueste Eintrag — er wird als einziger direkt ueber `EINTRAEGE[9]`
...
const ANSICHTMASSTAB = EINTRAEGE[9];
...
const SYMBOLMASSE = EINTRAEGE[10];
...
// er wird als einziger direkt ueber `EINTRAEGE[11]` geprueft; die bisherige Reihe rueckt geschlossen
...
const SAMMELPRODUKTE = EINTRAEGE[11];
...
const SPANNSYMBOLE = EINTRAEGE[12];
...
// es wird als einziges direkt ueber `EINTRAEGE[13]` geprueft; die bisherige Reihe rueckt geschlossen
...
const SAMMELMERKMALE = EINTRAEGE[13];
...
const EINLEGEMENGE = EINTRAEGE[14];
...
// `EINTRAEGE[15]` geprueft; die bisherige Reihe rueckt geschlossen um eins
...
const ZEICHNUNGSSYNC = EINTRAEGE[15];
...
// ueber `EINTRAEGE[16]` geprueft; die bisherige Reihe ist dafuer geschlossen um eins nach
...
const EDITIERMODUS = EINTRAEGE[16];
...
// direkt ueber `EINTRAEGE[17]` geprueft; die bisherige Reihe rueckt geschlossen um eins
...
const AUSGLEICHSMENGE = EINTRAEGE[17];
...
const MASSORIENTIERUNG = EINTRAEGE[18];
...
// als einzige direkt ueber `EINTRAEGE[19]` geprueft. Aussagewahr heisst hier: geliefert ist die
...
const AUSGLEICHSPUNKTE = EINTRAEGE[19];
...
const KATALOGMODUL = EINTRAEGE[20];
...
const PDFRUECKKEHR = EINTRAEGE[21];
...
const ACHSTREFFER = EINTRAEGE[22];
...
const HANDBUCHREGELN = EINTRAEGE[24];
...
// einziges direkt ueber `EINTRAEGE[25]` geprueft; die bisherige Reihe rueckt geschlossen um eins
...
// einzige direkt ueber `EINTRAEGE[26]` geprueft. Aussagewahr heisst hier: geliefert ist
...
// einziger direkt ueber `EINTRAEGE[27]` geprueft; die bisherige Reihe rueckt geschlossen um eins
...
// einziger direkt ueber `EINTRAEGE[29]` geprueft; die bisherige Reihe rueckt geschlossen um
...
// `EINTRAEGE[30]`, weil die Randregel der Spannachsen und die einheitliche Bezeichnung der
...
const EINBAUHOEHE = EINTRAEGE[30];
...
const EINBAULAGEN = EINTRAEGE[32];
...
const ZEICHNUNGSPDF = EINTRAEGE[33];
...
const VORSCHAUHOEHE = EINTRAEGE[34];
...
const STANDARDLAENGE = EINTRAEGE[35];
...
const KOPIERSCHUTZ = EINTRAEGE[36];
...
// zu drehen.
const VORHER = EINTRAEGE.slice(55);
const NEU = EINTRAEGE.slice(61);
const AKTUELL = EINTRAEGE.slice(66);
const AELTER = EINTRAEGE.slice(76);
...
const SPANNPLATTE = EINTRAEGE[38];
...
const WANDAUSWAHL = EINTRAEGE[39];
...
const VORRATSSATZ = EINTRAEGE[43];
...
const GESCHOSSMENGE = EINTRAEGE[45];
...
const TEILAUSWAHL = EINTRAEGE[46];
...
// unveraendert, es entsteht kein Feld und kein Versionssprung.
const IMPORTDIALOG = EINTRAEGE[47];
