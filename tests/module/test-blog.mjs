...
const EINKAUFSLISTE113 = EINTRAEGE[0];
...
const PLATTENDICKE97 = EINTRAEGE[1];
...
// er wird als einziger direkt ueber `EINTRAEGE[2]` geprueft; die bisherige Reihe rueckt
...
const PDFNAMENSFOLGE107 = EINTRAEGE[2];
...
// als einziger direkt ueber `EINTRAEGE[3]` geprueft; die bisherige Reihe rueckt geschlossen
...
const STDKATALOG113 = EINTRAEGE[3];
...
// Eintrag — er wird als einziger direkt ueber `EINTRAEGE[4]` geprueft; die bisherige Reihe
...
const BESCHBLOCK113 = EINTRAEGE[4];
...
// zweitneueste Eintrag — er wird als einziger direkt ueber `EINTRAEGE[6]` geprueft; die bisherige
...
const BLECHSTOSS91 = EINTRAEGE[6];
...
// viertneueste Eintrag — er wird als einziger direkt ueber `EINTRAEGE[8]` geprueft; die
...
// `EINTRAEGE[9]` geprueft; die bisherige Reihe rueckt geschlossen um eins nach hinten.
...
const DECKENZEIGEN = EINTRAEGE[9];
...
// aus #94) ist der neueste Eintrag — er wird als einziger direkt ueber `EINTRAEGE[10]` geprueft;
...
// chg-20260907-20) ist der neueste Eintrag — er wird als einziger direkt ueber `EINTRAEGE[11]`
...
const SAMMELMEHRFACH = EINTRAEGE[11];
...
// einziger direkt ueber `EINTRAEGE[12]` geprueft; die bisherige Reihe rueckt geschlossen um eins
...
const OHNE_SCHEIBE = EINTRAEGE[12];
...
// mit [A-3]/#97) sind der neueste Eintrag — er wird als einziger direkt ueber `EINTRAEGE[13]`
...
const ANSICHTMASSTAB = EINTRAEGE[13];
...
// sie werden als einzige direkt ueber `EINTRAEGE[14]` geprueft; die bisherige Reihe rueckt
...
const SYMBOLMASSE = EINTRAEGE[14];
...
// er wird als einziger direkt ueber `EINTRAEGE[15]` geprueft; die bisherige Reihe rueckt geschlossen
// um eins nach hinten. Aussagewahr heisst hier: geliefert ist AUSSCHLIESSLICH, dass die
...
const SAMMELPRODUKTE = EINTRAEGE[15];
...
const SPANNSYMBOLE = EINTRAEGE[16];
...
// es wird als einziges direkt ueber `EINTRAEGE[17]` geprueft; die bisherige Reihe rueckt geschlossen
...
const SAMMELMERKMALE = EINTRAEGE[17];
...
const EINLEGEMENGE = EINTRAEGE[18];
...
// `EINTRAEGE[19]` geprueft; die bisherige Reihe rueckt geschlossen um eins
...
const ZEICHNUNGSSYNC = EINTRAEGE[19];
...
// ueber `EINTRAEGE[20]` geprueft; die bisherige Reihe ist dafuer geschlossen um eins nach
...
const EDITIERMODUS = EINTRAEGE[20];
...
// direkt ueber `EINTRAEGE[21]` geprueft; die bisherige Reihe rueckt geschlossen um eins
...
const AUSGLEICHSMENGE = EINTRAEGE[21];
...
// `EINTRAEGE[22]` geprueft; die bisherige Reihe rueckt geschlossen um eins nach hinten.
...
const MASSORIENTIERUNG = EINTRAEGE[22];
...
// als einzige direkt ueber `EINTRAEGE[23]` geprueft. Aussagewahr heisst hier: geliefert ist die
...
const AUSGLEICHSPUNKTE = EINTRAEGE[23];
...
const KATALOGMODUL = EINTRAEGE[24];
...
const PDFRUECKKEHR = EINTRAEGE[25];
...
const ACHSTREFFER = EINTRAEGE[26];
...
const HANDBUCHREGELN = EINTRAEGE[28];
...
// einziges direkt ueber `EINTRAEGE[29]` geprueft; die bisherige Reihe rueckt geschlossen um eins
...
// einzige direkt ueber `EINTRAEGE[30]` geprueft. Aussagewahr heisst hier: geliefert ist
...
// einziger direkt ueber `EINTRAEGE[31]` geprueft; die bisherige Reihe rueckt geschlossen um eins
...
// einziger direkt ueber `EINTRAEGE[33]` geprueft; die bisherige Reihe rueckt geschlossen um
...
// `EINTRAEGE[34]`, weil die Randregel der Spannachsen und die einheitliche Bezeichnung der
...
const EINBAUHOEHE = EINTRAEGE[34];
...
ok("genau zwei Eintraege fuer Issue 94, neu vor alt",
  EINTRAEGE.filter(e => e.issue === 94).map(e => e.id).join(",")
    === "chg-20260907-08,chg-20260906-02");
...
const EINBAULAGEN = EINTRAEGE[36];
...
const ZEICHNUNGSPDF = EINTRAEGE[37];
...
const VORSCHAUHOEHE = EINTRAEGE[38];
...
const STANDARDLAENGE = EINTRAEGE[39];
...
const KOPIERSCHUTZ = EINTRAEGE[40];
...
// zu drehen.
const VORHER = EINTRAEGE.slice(59);
const NEU = EINTRAEGE.slice(65);
const AKTUELL = EINTRAEGE.slice(70);
const AELTER = EINTRAEGE.slice(80);
...
const SPANNPLATTE = EINTRAEGE[42];
...
const WANDAUSWAHL = EINTRAEGE[43];
...
const VORRATSSATZ = EINTRAEGE[47];
...
const GESCHOSSMENGE = EINTRAEGE[49];
...
const TEILAUSWAHL = EINTRAEGE[50];
...
// unveraendert, es entsteht kein Feld und kein Versionssprung.
const IMPORTDIALOG = EINTRAEGE[51];
