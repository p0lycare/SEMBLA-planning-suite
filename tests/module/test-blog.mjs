...
const BLECHSTOSS91 = EINTRAEGE[0];
ok("[#91] der weisse Bodenblechstoss ist der neueste Eintrag",
...
// zweitneueste Eintrag — er wird als einziger direkt ueber `EINTRAEGE[2]` geprueft; die
...
// `EINTRAEGE[3]` geprueft; die bisherige Reihe rueckt geschlossen um eins nach hinten.
...
const DECKENZEIGEN = EINTRAEGE[3];
...
// aus #94) ist der neueste Eintrag — er wird als einziger direkt ueber `EINTRAEGE[4]` geprueft;
...
// chg-20260907-20) ist der neueste Eintrag — er wird als einziger direkt ueber `EINTRAEGE[5]`
...
const SAMMELMEHRFACH = EINTRAEGE[5];
...
const OHNE_SCHEIBE = EINTRAEGE[6];
...
// mit [A-3]/#97) sind der neueste Eintrag — er wird als einziger direkt ueber `EINTRAEGE[7]`
...
const ANSICHTMASSTAB = EINTRAEGE[7];
...
// er wird als einziger direkt ueber `EINTRAEGE[9]` geprueft; die bisherige Reihe rueckt geschlossen
...
const SAMMELPRODUKTE = EINTRAEGE[9];
...
const SPANNSYMBOLE = EINTRAEGE[10];
...
// es wird als einziges direkt ueber `EINTRAEGE[11]` geprueft; die bisherige Reihe rueckt geschlossen
...
const SAMMELMERKMALE = EINTRAEGE[11];
...
const EINLEGEMENGE = EINTRAEGE[12];
...
// `EINTRAEGE[13]` geprueft; die bisherige Reihe rueckt geschlossen um eins
...
const ZEICHNUNGSSYNC = EINTRAEGE[13];
...
// ueber `EINTRAEGE[14]` geprueft; die bisherige Reihe ist dafuer geschlossen um eins nach
...
const EDITIERMODUS = EINTRAEGE[14];
...
// direkt ueber `EINTRAEGE[15]` geprueft; die bisherige Reihe rueckt geschlossen um eins
...
const AUSGLEICHSMENGE = EINTRAEGE[15];
...
const MASSORIENTIERUNG = EINTRAEGE[16];
...
// als einzige direkt ueber `EINTRAEGE[17]` geprueft. Aussagewahr heisst hier: geliefert ist die
...
const AUSGLEICHSPUNKTE = EINTRAEGE[17];
...
const KATALOGMODUL = EINTRAEGE[18];
...
const PDFRUECKKEHR = EINTRAEGE[19];
...
const ACHSTREFFER = EINTRAEGE[20];
...
const HANDBUCHREGELN = EINTRAEGE[22];
...
// einziges direkt ueber `EINTRAEGE[23]` geprueft; die bisherige Reihe rueckt geschlossen um eins
...
// einzige direkt ueber `EINTRAEGE[24]` geprueft. Aussagewahr heisst hier: geliefert ist
...
// einziger direkt ueber `EINTRAEGE[25]` geprueft; die bisherige Reihe rueckt geschlossen um eins
...
// einziger direkt ueber `EINTRAEGE[27]` geprueft; die bisherige Reihe rueckt geschlossen um
...
// `EINTRAEGE[28]`, weil die Randregel der Spannachsen und die einheitliche Bezeichnung der
...
const EINBAUHOEHE = EINTRAEGE[28];
...
const EINBAULAGEN = EINTRAEGE[30];
...
const ZEICHNUNGSPDF = EINTRAEGE[31];
...
const VORSCHAUHOEHE = EINTRAEGE[32];
...
const STANDARDLAENGE = EINTRAEGE[33];
...
const KOPIERSCHUTZ = EINTRAEGE[34];
...
// zu drehen.
const VORHER = EINTRAEGE.slice(53);
const NEU = EINTRAEGE.slice(59);
const AKTUELL = EINTRAEGE.slice(64);
const AELTER = EINTRAEGE.slice(74);
...
const SPANNPLATTE = EINTRAEGE[36];
...
const WANDAUSWAHL = EINTRAEGE[37];
...
const VORRATSSATZ = EINTRAEGE[41];
...
const GESCHOSSMENGE = EINTRAEGE[43];
...
const TEILAUSWAHL = EINTRAEGE[44];
...
// unveraendert, es entsteht kein Feld und kein Versionssprung.
const IMPORTDIALOG = EINTRAEGE[45];
