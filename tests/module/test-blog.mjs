...
ok("die Blattreduktion zu Issue 61 ist der neueste Eintrag",
  EINTRAEGE[0]?.id === "chg-20260812-04" && EINTRAEGE[0]?.issue === 61);
ok("der kompakte Lageplankopf zu Issue 59 folgt direkt danach",
  EINTRAEGE[1]?.id === "chg-20260812-03" && EINTRAEGE[1]?.issue === 59);
ok("der Eintrag zu Issue 56 bleibt als dritter aktueller Eintrag erhalten",
  EINTRAEGE[2]?.id === "chg-20260812-02" && EINTRAEGE[2]?.issue === 56);
ok("die Workflow-Retros zu Issue 65 bleiben als vierter aktueller Eintrag erhalten",
  EINTRAEGE[3]?.id === "chg-20260812-01" && EINTRAEGE[3]?.issue === 65);
