...
// Der EDITIERMODUS der Ausgleichspunkte (#96) ist der neueste Eintrag — er wird als einziger
// direkt ueber `EINTRAEGE[0]` geprueft; die bisherige Reihe rueckt geschlossen um eins nach
// hinten. Aussagewahr heisst hier: geliefert ist AUSSCHLIESSLICH die Bedienung in Modul 1 —
...
const EDITIERMODUS = EINTRAEGE[0];
...
// direkt ueber `EINTRAEGE[1]` geprueft; die bisherige Reihe rueckt geschlossen um eins
...
const AUSGLEICHSMENGE = EINTRAEGE[1];
...
const MASSORIENTIERUNG = EINTRAEGE[2];
...
// als einzige direkt ueber `EINTRAEGE[3]` geprueft. Aussagewahr heisst hier: geliefert ist die
...
const AUSGLEICHSPUNKTE = EINTRAEGE[3];
...
const KATALOGMODUL = EINTRAEGE[4];
...
const PDFRUECKKEHR = EINTRAEGE[5];
...
const HANDBUCHREGELN = EINTRAEGE[8];
...
// einziges direkt ueber `EINTRAEGE[9]` geprueft; die bisherige Reihe rueckt geschlossen um eins
...
// einzige direkt ueber `EINTRAEGE[10]` geprueft. Aussagewahr heisst hier: geliefert ist
...
// einziger direkt ueber `EINTRAEGE[13]` geprueft; die bisherige Reihe rueckt geschlossen um
...
// `EINTRAEGE[14]`, weil die Randregel der Spannachsen und die einheitliche Bezeichnung der
...
const EINBAUHOEHE = EINTRAEGE[14];
...
const EINBAULAGEN = EINTRAEGE[16];
...
const ZEICHNUNGSPDF = EINTRAEGE[17];
...
const VORSCHAUHOEHE = EINTRAEGE[18];
...
const STANDARDLAENGE = EINTRAEGE[19];
...
const KOPIERSCHUTZ = EINTRAEGE[20];
...
// zu drehen.
const VORHER = EINTRAEGE.slice(39);
const NEU = EINTRAEGE.slice(45);
const AKTUELL = EINTRAEGE.slice(50);
const AELTER = EINTRAEGE.slice(60);
...
const SPANNPLATTE = EINTRAEGE[22];
...
const WANDAUSWAHL = EINTRAEGE[23];
...
const VORRATSSATZ = EINTRAEGE[27];
...
const GESCHOSSMENGE = EINTRAEGE[29];
...
const TEILAUSWAHL = EINTRAEGE[30];
...
// unveraendert, es entsteht kein Feld und kein Versionssprung.
const IMPORTDIALOG = EINTRAEGE[31];
