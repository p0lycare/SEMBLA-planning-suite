...
// Der NEUESTE Eintrag sind die Zwischenspannpunkte (#93) — er wird als
...
// zu drehen.
const VORHER = EINTRAEGE.slice(18);
const NEU = EINTRAEGE.slice(24);
const AKTUELL = EINTRAEGE.slice(29);
const AELTER = EINTRAEGE.slice(39);
...
ok("die Zwischenspannpunkte (Issue 93) sind der neueste Eintrag",
  EINTRAEGE[0]?.id === "chg-20260904-09" && EINTRAEGE[0]?.issue === 93
...
ok("der Eintrag benennt beide Seiten: abgeleitete Vorgabe und eigene Bearbeitung",
  /automatisch/i.test(EINTRAEGE[0]?.titel || "")
  && /lagengenau/i.test(EINTRAEGE[0]?.titel || "")
  && /bearbeitbar/i.test(EINTRAEGE[0]?.titel || "")
  // Keine Stuecklisten-/Mengen-/Preis-/Katalog-/Nachweiszusage: das bleibt offen.
  && !/Stückliste|Menge|Preis|Katalog|Nachweis|Maß/i.test(EINTRAEGE[0]?.titel || ""));
ok("die Testbitte fuehrt den vollstaendigen Bedienweg samt Rueckkehr zu Auto",
  /Modul 1/.test(EINTRAEGE[0]?.testbitte || "")
  && /Oberkante/.test(EINTRAEGE[0]?.testbitte || "")
  && /ziehen/.test(EINTRAEGE[0]?.testbitte || "")
  && /löschen/.test(EINTRAEGE[0]?.testbitte || "")
  && /Zurück zu Auto/.test(EINTRAEGE[0]?.testbitte || ""));
...
const GESCHOSSMENGE = EINTRAEGE[8];
...
// unveraendert, es entsteht kein Feld und kein Versionssprung.
const IMPORTDIALOG = EINTRAEGE[10];
