...
// Die SCHLUESSELWEITE AM WANDELEMENT (#97) ist der neueste Eintrag — er wird als einziger
// direkt ueber `EINTRAEGE[0]` geprueft; die bisherige Reihe rueckt geschlossen um eins nach
// hinten. Aussagewahr heisst hier: geliefert ist AUSSCHLIESSLICH, dass Modul 1 die gepflegte
// Schluesselweite beim Auslegen an die Wand fuehrt und die Rechnung dabei unveraendert bleibt.
// Ausdruecklich NICHT versprochen wird eine ZEICHNUNG — die masstaebliche Mutternbreite ist ein
// eigenes Folgepaket, und der Eintrag muss das sagen statt es zu verschweigen. Ebenso wenig
// versprochen werden ein Preis, eine Menge, eine Norm, ein Nachweis oder ein Versionssprung.
const SW_WAND97 = EINTRAEGE[0];
...
// direkt ueber `EINTRAEGE[1]` geprueft; die bisherige Reihe rueckt geschlossen um eins nach
...
const SCHLUESSELWEITE97 = EINTRAEGE[1];
...
// als einziger direkt ueber `EINTRAEGE[2]` geprueft; die bisherige Reihe rueckt geschlossen um
...
const NEURECHNUNG117 = EINTRAEGE[2];
...
// als einziger direkt ueber `EINTRAEGE[3]` geprueft; die bisherige Reihe rueckt geschlossen um
...
const FEHLREFERENZ116 = EINTRAEGE[3];
...
// einziger direkt ueber `EINTRAEGE[4]` geprueft; die bisherige Reihe rueckt geschlossen um
...
const STANGENVORKOPPLUNG112 = EINTRAEGE[4];
...
// wird als einziger direkt ueber `EINTRAEGE[5]` geprueft; die bisherige Reihe rueckt
...
const ALTBESTAND114 = EINTRAEGE[5];
...
// direkt ueber `EINTRAEGE[6]` geprueft; die bisherige Reihe rueckt geschlossen um eins nach
...
const EBENENNACHWEIS94 = EINTRAEGE[6];
...
// einziger direkt ueber `EINTRAEGE[7]` geprueft; die bisherige Reihe rueckt geschlossen um eins
...
const MUTTERNHOEHE97 = EINTRAEGE[7];
...
// direkt ueber `EINTRAEGE[8]` geprueft; die bisherige Reihe rueckt geschlossen um eins nach
...
const BAUGRUPPENANSICHT94 = EINTRAEGE[8];
...
// als einziger direkt ueber `EINTRAEGE[9]` geprueft; die bisherige Reihe rueckt geschlossen um
...
const EINKAUFSLISTE113 = EINTRAEGE[9];
...
// einziger direkt ueber `EINTRAEGE[10]` geprueft; die bisherige Reihe rueckt geschlossen um
...
const PLATTENDICKE97 = EINTRAEGE[10];
...
// (chg-20260909-14) und ihre Ableitung ans Wandelement (chg-20260909-15). Je Paket GENAU EIN
// Eintrag — und keiner mehr, damit dieselbe Aenderung nicht zweimal in der Liste steht. Die
// beiden Schluesselweiten-Eintraege sind ausdruecklich ZWEI Pakete: das eine pflegt das Mass im
// Katalog, das andere fuehrt es an die Wand.
...
// er wird als einziger direkt ueber `EINTRAEGE[11]` geprueft; die bisherige Reihe rueckt
// geschlossen um eins nach hinten. Aussagewahr heisst hier: geliefert ist AUSSCHLIESSLICH,
...
const PDFNAMENSFOLGE107 = EINTRAEGE[11];
...
// als einziger direkt ueber `EINTRAEGE[12]` geprueft; die bisherige Reihe rueckt geschlossen
...
const STDKATALOG113 = EINTRAEGE[12];
...
// Eintrag — er wird als einziger direkt ueber `EINTRAEGE[13]` geprueft; die bisherige Reihe
...
const BESCHBLOCK113 = EINTRAEGE[13];
...
// er wird als einziger direkt ueber `EINTRAEGE[14]` geprueft; die bisherige Reihe rueckt
// geschlossen um eins nach hinten. Aussagewahr heisst hier: geliefert ist AUSSCHLIESSLICH,
...
const VARIANTE108 = EINTRAEGE[14];
...
// zweitneueste Eintrag — er wird als einziger direkt ueber `EINTRAEGE[15]` geprueft; die bisherige
...
const BLECHSTOSS91 = EINTRAEGE[15];
...
// er wird als einziger direkt ueber `EINTRAEGE[16]` geprueft; die bisherige Reihe rueckt
// geschlossen um eins nach hinten. Aussagewahr heisst hier: geliefert ist AUSSCHLIESSLICH,
...
const BESCHAFFUNG = EINTRAEGE[16];
...
// viertneueste Eintrag — er wird als einziger direkt ueber `EINTRAEGE[17]` geprueft; die
...
const STANGENVORN = EINTRAEGE[17];
...
// `EINTRAEGE[18]` geprueft; die bisherige Reihe rueckt geschlossen um eins nach hinten.
...
const DECKENZEIGEN = EINTRAEGE[18];
...
// aus #94) ist der neueste Eintrag — er wird als einziger direkt ueber `EINTRAEGE[19]` geprueft;
...
// chg-20260907-20) ist der neueste Eintrag — er wird als einziger direkt ueber `EINTRAEGE[20]`
...
const SAMMELMEHRFACH = EINTRAEGE[20];
...
// einziger direkt ueber `EINTRAEGE[21]` geprueft; die bisherige Reihe rueckt geschlossen um eins
...
const OHNE_SCHEIBE = EINTRAEGE[21];
...
// mit [A-3]/#97) sind der neueste Eintrag — er wird als einziger direkt ueber `EINTRAEGE[22]`
...
const ANSICHTMASSTAB = EINTRAEGE[22];
...
// sie werden als einzige direkt ueber `EINTRAEGE[23]` geprueft; die bisherige Reihe rueckt
...
const SYMBOLMASSE = EINTRAEGE[23];
...
// er wird als einziger direkt ueber `EINTRAEGE[24]` geprueft; die bisherige Reihe rueckt geschlossen
// um eins nach hinten. Aussagewahr heisst hier: geliefert ist AUSSCHLIESSLICH, dass die
...
const SAMMELPRODUKTE = EINTRAEGE[24];
...
// sie werden als einzige direkt ueber `EINTRAEGE[25]` geprueft; die bisherige Reihe rueckt
// geschlossen um eins nach hinten. Aussagewahr heisst hier: geliefert ist AUSSCHLIESSLICH, dass
...
const SPANNSYMBOLE = EINTRAEGE[25];
...
// es wird als einziges direkt ueber `EINTRAEGE[26]` geprueft; die bisherige Reihe rueckt geschlossen
...
const SAMMELMERKMALE = EINTRAEGE[26];
...
// sie werden als einzige direkt ueber `EINTRAEGE[27]` geprueft; die bisherige Reihe rueckt
// geschlossen um eins nach hinten. Aussagewahr heisst hier: geliefert ist AUSSCHLIESSLICH, dass
...
const EINLEGEMENGE = EINTRAEGE[27];
...
// `EINTRAEGE[28]` geprueft; die bisherige Reihe rueckt geschlossen um eins
...
const ZEICHNUNGSSYNC = EINTRAEGE[28];
...
// ueber `EINTRAEGE[29]` geprueft; die bisherige Reihe ist dafuer geschlossen um eins nach
...
const EDITIERMODUS = EINTRAEGE[29];
...
// direkt ueber `EINTRAEGE[30]` geprueft; die bisherige Reihe rueckt geschlossen um eins
...
const AUSGLEICHSMENGE = EINTRAEGE[30];
...
// `EINTRAEGE[31]` geprueft; die bisherige Reihe rueckt geschlossen um eins nach hinten.
...
const MASSORIENTIERUNG = EINTRAEGE[31];
...
// als einzige direkt ueber `EINTRAEGE[32]` geprueft. Aussagewahr heisst hier: geliefert ist die
...
const AUSGLEICHSPUNKTE = EINTRAEGE[32];
...
const KATALOGMODUL = EINTRAEGE[33];
...
const PDFRUECKKEHR = EINTRAEGE[34];
...
const ACHSTREFFER = EINTRAEGE[35];
...
const HANDBUCHREGELN = EINTRAEGE[37];
...
// einziges direkt ueber `EINTRAEGE[38]` geprueft; die bisherige Reihe rueckt geschlossen um eins
...
// einzige direkt ueber `EINTRAEGE[39]` geprueft. Aussagewahr heisst hier: geliefert ist
...
// einziger direkt ueber `EINTRAEGE[40]` geprueft; die bisherige Reihe rueckt geschlossen um eins
...
// einziger direkt ueber `EINTRAEGE[42]` geprueft; die bisherige Reihe rueckt geschlossen um
...
// `EINTRAEGE[43]`, weil die Randregel der Spannachsen und die einheitliche Bezeichnung der
...
const EINBAUHOEHE = EINTRAEGE[43];
...
const EINBAULAGEN = EINTRAEGE[45];
...
const ZEICHNUNGSPDF = EINTRAEGE[46];
...
const VORSCHAUHOEHE = EINTRAEGE[47];
...
const STANDARDLAENGE = EINTRAEGE[48];
...
const KOPIERSCHUTZ = EINTRAEGE[49];
...
// zu drehen.
const VORHER = EINTRAEGE.slice(68);
const NEU = EINTRAEGE.slice(74);
const AKTUELL = EINTRAEGE.slice(79);
const AELTER = EINTRAEGE.slice(89);
...
const SPANNPLATTE = EINTRAEGE[51];
...
const WANDAUSWAHL = EINTRAEGE[52];
...
const VORRATSSATZ = EINTRAEGE[56];
...
const GESCHOSSMENGE = EINTRAEGE[58];
...
const TEILAUSWAHL = EINTRAEGE[59];
...
// unveraendert, es entsteht kein Feld und kein Versionssprung.
const IMPORTDIALOG = EINTRAEGE[60];
