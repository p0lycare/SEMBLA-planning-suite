...
// Die VOLLSTAENDIGE NEURECHNUNG der Sammelaenderung (#117) ist der neueste Eintrag — er wird
// als einziger direkt ueber `EINTRAEGE[0]` geprueft; die bisherige Reihe rueckt geschlossen um
...
const NEURECHNUNG117 = EINTRAEGE[0];
...
const FEHLREFERENZ116 = EINTRAEGE[1];
...
// einziger direkt ueber `EINTRAEGE[2]` geprueft; die bisherige Reihe rueckt geschlossen um
...
const STANGENVORKOPPLUNG112 = EINTRAEGE[2];
...
// wird als einziger direkt ueber `EINTRAEGE[3]` geprueft; die bisherige Reihe rueckt
...
const ALTBESTAND114 = EINTRAEGE[3];
...
// direkt ueber `EINTRAEGE[4]` geprueft; die bisherige Reihe rueckt geschlossen um eins nach
...
const EBENENNACHWEIS94 = EINTRAEGE[4];
...
const MUTTERNHOEHE97 = EINTRAEGE[5];
...
const BAUGRUPPENANSICHT94 = EINTRAEGE[6];
...
// als einziger direkt ueber `EINTRAEGE[7]` geprueft; die bisherige Reihe rueckt geschlossen um
...
const EINKAUFSLISTE113 = EINTRAEGE[7];
...
// einziger direkt ueber `EINTRAEGE[8]` geprueft; die bisherige Reihe rueckt geschlossen um
...
const PLATTENDICKE97 = EINTRAEGE[8];
...
// Issue 97 wird in mehreren Paketen geliefert: die Spannplattendicke (chg-20260909-05) und
// die Einbauhoehe der Kopplungsmutter (chg-20260909-08). Je Paket GENAU EIN Eintrag — und
// keiner mehr, damit dieselbe Aenderung nicht zweimal in der Liste steht.
...
// er wird als einziger direkt ueber `EINTRAEGE[9]` geprueft; die bisherige Reihe rueckt
// geschlossen um eins nach hinten. Aussagewahr heisst hier: geliefert ist AUSSCHLIESSLICH,
...
const PDFNAMENSFOLGE107 = EINTRAEGE[9];
...
// als einziger direkt ueber `EINTRAEGE[10]` geprueft; die bisherige Reihe rueckt geschlossen
...
const STDKATALOG113 = EINTRAEGE[10];
...
// Eintrag — er wird als einziger direkt ueber `EINTRAEGE[11]` geprueft; die bisherige Reihe
...
const BESCHBLOCK113 = EINTRAEGE[11];
...
const VARIANTE108 = EINTRAEGE[12];
...
// zweitneueste Eintrag — er wird als einziger direkt ueber `EINTRAEGE[13]` geprueft; die bisherige
...
const BLECHSTOSS91 = EINTRAEGE[13];
...
const BESCHAFFUNG = EINTRAEGE[14];
...
// viertneueste Eintrag — er wird als einziger direkt ueber `EINTRAEGE[15]` geprueft; die
...
const STANGENVORN = EINTRAEGE[15];
...
// `EINTRAEGE[16]` geprueft; die bisherige Reihe rueckt geschlossen um eins nach hinten.
...
const DECKENZEIGEN = EINTRAEGE[16];
...
// aus #94) ist der neueste Eintrag — er wird als einziger direkt ueber `EINTRAEGE[17]` geprueft;
...
// chg-20260907-20) ist der neueste Eintrag — er wird als einziger direkt ueber `EINTRAEGE[18]`
...
const SAMMELMEHRFACH = EINTRAEGE[18];
...
// einziger direkt ueber `EINTRAEGE[19]` geprueft; die bisherige Reihe rueckt geschlossen um eins
...
const OHNE_SCHEIBE = EINTRAEGE[19];
...
// mit [A-3]/#97) sind der neueste Eintrag — er wird als einziger direkt ueber `EINTRAEGE[20]`
...
const ANSICHTMASSTAB = EINTRAEGE[20];
...
// sie werden als einzige direkt ueber `EINTRAEGE[21]` geprueft; die bisherige Reihe rueckt
...
const SYMBOLMASSE = EINTRAEGE[21];
...
// er wird als einziger direkt ueber `EINTRAEGE[22]` geprueft; die bisherige Reihe rueckt geschlossen
// um eins nach hinten. Aussagewahr heisst hier: geliefert ist AUSSCHLIESSLICH, dass die
...
const SAMMELPRODUKTE = EINTRAEGE[22];
...
const SPANNSYMBOLE = EINTRAEGE[23];
...
// es wird als einziges direkt ueber `EINTRAEGE[24]` geprueft; die bisherige Reihe rueckt geschlossen
...
const SAMMELMERKMALE = EINTRAEGE[24];
...
const EINLEGEMENGE = EINTRAEGE[25];
...
// `EINTRAEGE[26]` geprueft; die bisherige Reihe rueckt geschlossen um eins
...
const ZEICHNUNGSSYNC = EINTRAEGE[26];
...
// ueber `EINTRAEGE[27]` geprueft; die bisherige Reihe ist dafuer geschlossen um eins nach
...
const EDITIERMODUS = EINTRAEGE[27];
...
// direkt ueber `EINTRAEGE[28]` geprueft; die bisherige Reihe rueckt geschlossen um eins
...
const AUSGLEICHSMENGE = EINTRAEGE[28];
...
// `EINTRAEGE[29]` geprueft; die bisherige Reihe rueckt geschlossen um eins nach hinten.
...
const MASSORIENTIERUNG = EINTRAEGE[29];
...
// als einzige direkt ueber `EINTRAEGE[30]` geprueft. Aussagewahr heisst hier: geliefert ist die
...
const AUSGLEICHSPUNKTE = EINTRAEGE[30];
...
const KATALOGMODUL = EINTRAEGE[31];
...
const PDFRUECKKEHR = EINTRAEGE[32];
...
const ACHSTREFFER = EINTRAEGE[33];
...
const HANDBUCHREGELN = EINTRAEGE[35];
...
// einziges direkt ueber `EINTRAEGE[36]` geprueft; die bisherige Reihe rueckt geschlossen um eins
...
// einzige direkt ueber `EINTRAEGE[37]` geprueft. Aussagewahr heisst hier: geliefert ist
...
// einziger direkt ueber `EINTRAEGE[38]` geprueft; die bisherige Reihe rueckt geschlossen um eins
...
// einziger direkt ueber `EINTRAEGE[40]` geprueft; die bisherige Reihe rueckt geschlossen um
...
// `EINTRAEGE[41]`, weil die Randregel der Spannachsen und die einheitliche Bezeichnung der
...
const EINBAUHOEHE = EINTRAEGE[41];
...
const EINBAULAGEN = EINTRAEGE[43];
...
const ZEICHNUNGSPDF = EINTRAEGE[44];
...
const VORSCHAUHOEHE = EINTRAEGE[45];
...
const STANDARDLAENGE = EINTRAEGE[46];
...
const KOPIERSCHUTZ = EINTRAEGE[47];
...
// zu drehen.
const VORHER = EINTRAEGE.slice(66);
const NEU = EINTRAEGE.slice(72);
const AKTUELL = EINTRAEGE.slice(77);
const AELTER = EINTRAEGE.slice(87);
...
const SPANNPLATTE = EINTRAEGE[49];
...
const WANDAUSWAHL = EINTRAEGE[50];
...
const VORRATSSATZ = EINTRAEGE[54];
...
const GESCHOSSMENGE = EINTRAEGE[56];
...
const TEILAUSWAHL = EINTRAEGE[57];
...
// unveraendert, es entsteht kein Feld und kein Versionssprung.
const IMPORTDIALOG = EINTRAEGE[58];
