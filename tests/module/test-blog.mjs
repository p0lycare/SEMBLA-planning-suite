...
// #82 (Verzahnungsbereiche in Modul 1) ist der NEUESTE Eintrag, #59 (ueberdeckungsfreie
// Nummernblasen) der zweit-, #81 (waehlbare Mengenfassung) der dritt-, #79 (technische
// Wandzeichnung) der viert-, #79 (Geschosseditor) der fuenft-, #79 (Lageplan) der
// sechst-, #81 (Mengenuebersteuerung) der siebt-, #79 (Wahl in Modul 1) der acht- und
// #80 der neuntneueste. Die folgenden Zusicherungen prüfen die Reihenfolge der älteren
// Einträge und zählen deshalb ab dem zehnten — so bleibt jede Positionsaussage erhalten,
// ohne 60 Indizes zu drehen.
const AELTER = EINTRAEGE.slice(9);
...
// #81 hat ZWEI Eintraege, weil es zwei getrennte Nutzerergebnisse waren: erst die
// manuelle Menge in Modul 4, dann die waehlbare Mengenfassung im zentralen Export.
const neu81 = EINTRAEGE.filter(e => e.issue === 81);
ok("genau zwei Eintraege fuer Issue 81 — Uebersteuerung in Modul 4 und Fassungswahl im Export",
  neu81.length === 2
  && neu81.map(e => e.id).join(",") === "chg-20260816-02,chg-20260815-01");
ok("die Verzahnungsbereiche in Modul 1 (Issue 82) sind der neueste Eintrag",
  EINTRAEGE[0]?.id === "chg-20260816-04" && EINTRAEGE[0]?.issue === 82
...
ok("der Verzahnungs-Eintrag benennt Ort, Verband und Mengenwirkung aussagewahr",
  /Modul 1/.test(EINTRAEGE[0]?.titel || "")
  && /Verzahnungsbereiche/.test(EINTRAEGE[0]?.titel || "")
  && /alternierend/.test(EINTRAEGE[0]?.titel || "")
  && /Mengen/.test(EINTRAEGE[0]?.titel || ""));
ok("die Verzahnungs-Testbitte benennt Ort, Lage, Mengen und unveraenderte Vorspannung",
  /Modul 1/.test(EINTRAEGE[0]?.testbitte || "")
  && /Verzahnungsbereich/.test(EINTRAEGE[0]?.testbitte || "")
  && /jeder zweiten Lage/.test(EINTRAEGE[0]?.testbitte || "")
  && /Steinmengen/.test(EINTRAEGE[0]?.testbitte || "")
  && /Vorspannung/.test(EINTRAEGE[0]?.testbitte || "")
  && /unverändert/.test(EINTRAEGE[0]?.testbitte || ""));
ok("die ueberdeckungsfreien Nummernblasen im Lageplan (Issue 59) folgen direkt danach",
  EINTRAEGE[1]?.id === "chg-20260816-03" && EINTRAEGE[1]?.issue === 59
...
ok("der Blasen-Eintrag benennt Ausweichen, Ort und die beiden gemiedenen Dinge aussagewahr",
  ...
  && /weichen/.test(EINTRAEGE[1]?.titel || "")
  && /überdecken/.test(EINTRAEGE[1]?.titel || "")
  && /Bemaßung/.test(EINTRAEGE[1]?.titel || "")
  && !/verschieb/i.test(EINTRAEGE[1]?.titel || ""));
ok("die Blasen-Testbitte benennt Ausgangslage, Wandkante und die drei Ausgaben",
  /Modul 9/.test(EINTRAEGE[1]?.testbitte || "")
  && /bemaßten Wänden/.test(EINTRAEGE[1]?.testbitte || "")
  && /Führungslinie/.test(EINTRAEGE[1]?.testbitte || "")
  && /Wandkante/.test(EINTRAEGE[1]?.testbitte || "")
  && /SVG-Datei/.test(EINTRAEGE[1]?.testbitte || ""));
ok("die waehlbare Mengenfassung im Export (Issue 81) folgt direkt danach",
  EINTRAEGE[2]?.id === "chg-20260816-02" && EINTRAEGE[2]?.issue === 81
...
ok("der Fassungs-Eintrag benennt beide Fassungen und den Ort der Wahl aussagewahr",
  /Export/.test(EINTRAEGE[2]?.titel || "")
  && /wählbar/.test(EINTRAEGE[2]?.titel || "")
  && /berechneten/.test(EINTRAEGE[2]?.titel || "")
  && /angepassten/.test(EINTRAEGE[2]?.titel || "")
  && /Stückliste/.test(EINTRAEGE[2]?.titel || ""));
ok("die Fassungs-Testbitte benennt Schreibort, Wahl, Dateikopf und die erhaltene Rechnung",
  ...
  && /angepasst/.test(EINTRAEGE[2]?.testbitte || "")
  && /Dateikopf/.test(EINTRAEGE[2]?.testbitte || "")
  && /berechnete Menge/.test(EINTRAEGE[2]?.testbitte || ""));
ok("die Darstellung in der technischen Zeichnung (Issue 79) folgt direkt danach",
  EINTRAEGE[3]?.id === "chg-20260816-01" && EINTRAEGE[3]?.issue === 79
...
ok("der Zeichnungs-Eintrag benennt beide Klassen, Kurztext und Legende aussagewahr",
  /F0\/F30|F0 und F30/.test(EINTRAEGE[3]?.titel || "")
  && /Wandzeichnung/.test(EINTRAEGE[3]?.titel || "")
  && /Kurztext/.test(EINTRAEGE[3]?.titel || "")
  && /Legende/.test(EINTRAEGE[3]?.titel || "")
  && !/schraffiert/.test(EINTRAEGE[3]?.titel || ""));
...
ok("die Darstellung im Geschosseditor (Issue 79) folgt direkt danach",
  EINTRAEGE[4]?.id === "chg-20260815-03" && EINTRAEGE[4]?.issue === 79
  && EINTRAEGE[4]?.typ === "feature" && EINTRAEGE[4]?.datum === "2026-08-15");
...
ok("der Geschosseditor-Eintrag benennt beide Klassen, Schraffur, Legende und Wandliste aussagewahr",
  ...
  && /Wandliste/.test(EINTRAEGE[4]?.titel || ""));
ok("die Geschosseditor-Testbitte benennt Wahlort, beide Klassen, Wandliste und Schwarz-Weiss",
  ...
  && /F30-Wand/.test(EINTRAEGE[4]?.testbitte || "")
  && /F0-Wände/.test(EINTRAEGE[4]?.testbitte || "")
  && /Wandliste/.test(EINTRAEGE[4]?.testbitte || "")
  && /schwarz-weiß/.test(EINTRAEGE[4]?.testbitte || ""));
ok("die Lageplan-Darstellung (Issue 79) folgt direkt danach",
  EINTRAEGE[5]?.id === "chg-20260815-02" && EINTRAEGE[5]?.issue === 79
...
ok("der Lageplan-Eintrag benennt beide Klassen, Schraffur, Beschriftung und Legende aussagewahr",
  ...
  && /Lageplan/.test(EINTRAEGE[5]?.titel || "")
...
ok("die Lageplan-Testbitte benennt Wahlort, Ausgabe, Wandliste und Schwarz-Weiss",
  ...
  && /Modul 9/.test(EINTRAEGE[5]?.testbitte || "")
  && /exportieren/.test(EINTRAEGE[5]?.testbitte || "")
...
ok("die Mengenuebersteuerung (Issue 81) folgt direkt danach",
  EINTRAEGE[6]?.id === "chg-20260815-01" && EINTRAEGE[6]?.issue === 81
...
ok("der Mengen-Eintrag benennt Uebersteuerung UND erhaltenen Originalwert aussagewahr",
  /Menge je Stücklistenposition/.test(EINTRAEGE[6]?.titel || "")
  && /manuell/.test(EINTRAEGE[6]?.titel || "")
  && /berechnete Menge/.test(EINTRAEGE[6]?.titel || "")
  && /daneben sichtbar/.test(EINTRAEGE[6]?.titel || ""));
ok("die Mengen-Testbitte benennt Anzeige, Persistenz, Ruecknahme und Abweisung",
  /Modul 4/.test(EINTRAEGE[6]?.testbitte || "")
  && /nebeneinander/.test(EINTRAEGE[6]?.testbitte || "")
  && /Neuladen/.test(EINTRAEGE[6]?.testbitte || "")
  && /zurücksetzen/.test(EINTRAEGE[6]?.testbitte || "")
  && /abgewiesen/.test(EINTRAEGE[6]?.testbitte || ""));
ok("die Brandschutz-Wahl in Modul 1 (Issue 79) folgt direkt danach",
  EINTRAEGE[7]?.id === "chg-20260814-04" && EINTRAEGE[7]?.issue === 79
  && EINTRAEGE[7]?.typ === "feature" && EINTRAEGE[7]?.datum === "2026-08-14");
...
ok("der Brandschutz-Eintrag benennt beide Klassen, den Ort der Wahl und den Standard aussagewahr",
  /F0/.test(EINTRAEGE[7]?.titel || "") && /F30/.test(EINTRAEGE[7]?.titel || "")
  && /Modul 1/.test(EINTRAEGE[7]?.titel || "")
  && /Standard F0/.test(EINTRAEGE[7]?.titel || "")
  && /kein Nachweis/.test(EINTRAEGE[7]?.titel || ""));
ok("die Brandschutz-Testbitte benennt Fortbestand, Neuberechnung und unveraenderte Ableitung",
  /neu laden/.test(EINTRAEGE[7]?.testbitte || "")
  && /exportieren/.test(EINTRAEGE[7]?.testbitte || "")
  && /Geschosseditor/.test(EINTRAEGE[7]?.testbitte || "")
  && /Länge ändern/.test(EINTRAEGE[7]?.testbitte || "")
  && /Stückliste/.test(EINTRAEGE[7]?.testbitte || ""));
ok("der Planhintergrund im Lageplan (Issue 80) folgt direkt danach",
  EINTRAEGE[8]?.id === "chg-20260814-03" && EINTRAEGE[8]?.issue === 80
...
ok("der Planhintergrund-Eintrag benennt Quelle, Einstellung und Ausgabe aussagewahr",
  /Geschossplan/.test(EINTRAEGE[8]?.titel || "")
  && /kalibriert/i.test(EINTRAEGE[8]?.titel || "")
  && /Hintergrund/.test(EINTRAEGE[8]?.titel || "")
  && /Transparenz/.test(EINTRAEGE[8]?.titel || "")
  && /Modul 9/.test(EINTRAEGE[8]?.testbitte || "")
  && /exportieren/.test(EINTRAEGE[8]?.testbitte || "")
  && /100 %/.test(EINTRAEGE[8]?.testbitte || ""));
