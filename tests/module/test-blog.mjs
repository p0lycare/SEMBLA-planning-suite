...
// direkt ueber `EINTRAEGE[2]` geprueft; die bisherige Reihe rueckt geschlossen um eins nach
...
const SCHLUESSELWEITE97 = EINTRAEGE[2];
...
const NEURECHNUNG117 = EINTRAEGE[3];
...
const FEHLREFERENZ116 = EINTRAEGE[4];
...
// einziger direkt ueber `EINTRAEGE[5]` geprueft; die bisherige Reihe rueckt geschlossen um
...
const STANGENVORKOPPLUNG112 = EINTRAEGE[5];
...
// wird als einziger direkt ueber `EINTRAEGE[6]` geprueft; die bisherige Reihe rueckt
...
const ALTBESTAND114 = EINTRAEGE[6];
...
// direkt ueber `EINTRAEGE[7]` geprueft; die bisherige Reihe rueckt geschlossen um eins nach
...
const EBENENNACHWEIS94 = EINTRAEGE[7];
...
// einziger direkt ueber `EINTRAEGE[8]` geprueft; die bisherige Reihe rueckt geschlossen um eins
...
const MUTTERNHOEHE97 = EINTRAEGE[8];
...
// direkt ueber `EINTRAEGE[9]` geprueft; die bisherige Reihe rueckt geschlossen um eins nach
...
const BAUGRUPPENANSICHT94 = EINTRAEGE[9];
...
// als einziger direkt ueber `EINTRAEGE[10]` geprueft; die bisherige Reihe rueckt geschlossen um
...
const EINKAUFSLISTE113 = EINTRAEGE[10];
...
// einziger direkt ueber `EINTRAEGE[11]` geprueft; die bisherige Reihe rueckt geschlossen um
...
const PLATTENDICKE97 = EINTRAEGE[11];
...
// (chg-20260909-14), ihre Ableitung ans Wandelement (chg-20260909-15) und die masstabsgetreue
// Mutternbreite (chg-20260909-16). Je Paket GENAU EIN Eintrag — und keiner mehr, damit
// dieselbe Aenderung nicht zweimal in der Liste steht. Die drei letzten sind ausdruecklich
// DREI Pakete: das erste pflegt das Mass im Katalog, das zweite fuehrt es an die Wand, das
// dritte zeichnet es.
...
// er wird als einziger direkt ueber `EINTRAEGE[12]` geprueft; die bisherige Reihe rueckt
// geschlossen um eins nach hinten. Aussagewahr heisst hier: geliefert ist AUSSCHLIESSLICH,
...
const PDFNAMENSFOLGE107 = EINTRAEGE[12];
...
// als einziger direkt ueber `EINTRAEGE[13]` geprueft; die bisherige Reihe rueckt geschlossen
...
const STDKATALOG113 = EINTRAEGE[13];
...
// Eintrag — er wird als einziger direkt ueber `EINTRAEGE[14]` geprueft; die bisherige Reihe
...
const BESCHBLOCK113 = EINTRAEGE[14];
...
// er wird als einziger direkt ueber `EINTRAEGE[15]` geprueft; die bisherige Reihe rueckt
// geschlossen um eins nach hinten. Aussagewahr heisst hier: geliefert ist AUSSCHLIESSLICH,
...
const VARIANTE108 = EINTRAEGE[15];
...
// zweitneueste Eintrag — er wird als einziger direkt ueber `EINTRAEGE[16]` geprueft; die bisherige
...
const BLECHSTOSS91 = EINTRAEGE[16];
...
// er wird als einziger direkt ueber `EINTRAEGE[17]` geprueft; die bisherige Reihe rueckt
// geschlossen um eins nach hinten. Aussagewahr heisst hier: geliefert ist AUSSCHLIESSLICH,
...
const BESCHAFFUNG = EINTRAEGE[17];
...
// viertneueste Eintrag — er wird als einziger direkt ueber `EINTRAEGE[18]` geprueft; die
...
const STANGENVORN = EINTRAEGE[18];
...
// `EINTRAEGE[19]` geprueft; die bisherige Reihe rueckt geschlossen um eins nach hinten.
...
const DECKENZEIGEN = EINTRAEGE[19];
...
// aus #94) ist der neueste Eintrag — er wird als einziger direkt ueber `EINTRAEGE[20]` geprueft;
...
// chg-20260907-20) ist der neueste Eintrag — er wird als einziger direkt ueber `EINTRAEGE[21]`
...
const SAMMELMEHRFACH = EINTRAEGE[21];
...
// einziger direkt ueber `EINTRAEGE[22]` geprueft; die bisherige Reihe rueckt geschlossen um eins
...
const OHNE_SCHEIBE = EINTRAEGE[22];
...
// mit [A-3]/#97) sind der neueste Eintrag — er wird als einziger direkt ueber `EINTRAEGE[23]`
...
const ANSICHTMASSTAB = EINTRAEGE[23];
...
// sie werden als einzige direkt ueber `EINTRAEGE[24]` geprueft; die bisherige Reihe rueckt
...
const SYMBOLMASSE = EINTRAEGE[24];
...
// er wird als einziger direkt ueber `EINTRAEGE[25]` geprueft; die bisherige Reihe rueckt geschlossen
// um eins nach hinten. Aussagewahr heisst hier: geliefert ist AUSSCHLIESSLICH, dass die
...
const SAMMELPRODUKTE = EINTRAEGE[25];
...
// sie werden als einzige direkt ueber `EINTRAEGE[26]` geprueft; die bisherige Reihe rueckt
// geschlossen um eins nach hinten. Aussagewahr heisst hier: geliefert ist AUSSCHLIESSLICH, dass
...
const SPANNSYMBOLE = EINTRAEGE[26];
...
// es wird als einziges direkt ueber `EINTRAEGE[27]` geprueft; die bisherige Reihe rueckt geschlossen
...
const SAMMELMERKMALE = EINTRAEGE[27];
...
// sie werden als einzige direkt ueber `EINTRAEGE[28]` geprueft; die bisherige Reihe rueckt
// geschlossen um eins nach hinten. Aussagewahr heisst hier: geliefert ist AUSSCHLIESSLICH, dass
...
const EINLEGEMENGE = EINTRAEGE[28];
...
// `EINTRAEGE[29]` geprueft; die bisherige Reihe rueckt geschlossen um eins
...
const ZEICHNUNGSSYNC = EINTRAEGE[29];
...
// ueber `EINTRAEGE[30]` geprueft; die bisherige Reihe ist dafuer geschlossen um eins nach
...
const EDITIERMODUS = EINTRAEGE[30];
...
// direkt ueber `EINTRAEGE[31]` geprueft; die bisherige Reihe rueckt geschlossen um eins
...
const AUSGLEICHSMENGE = EINTRAEGE[31];
...
// `EINTRAEGE[32]` geprueft; die bisherige Reihe rueckt geschlossen um eins nach hinten.
...
const MASSORIENTIERUNG = EINTRAEGE[32];
...
// als einzige direkt ueber `EINTRAEGE[33]` geprueft. Aussagewahr heisst hier: geliefert ist die
...
const AUSGLEICHSPUNKTE = EINTRAEGE[33];
...
const KATALOGMODUL = EINTRAEGE[34];
...
const PDFRUECKKEHR = EINTRAEGE[35];
...
const ACHSTREFFER = EINTRAEGE[36];
...
const HANDBUCHREGELN = EINTRAEGE[38];
...
// einziges direkt ueber `EINTRAEGE[39]` geprueft; die bisherige Reihe rueckt geschlossen um eins
...
// einzige direkt ueber `EINTRAEGE[40]` geprueft. Aussagewahr heisst hier: geliefert ist
...
// einziger direkt ueber `EINTRAEGE[41]` geprueft; die bisherige Reihe rueckt geschlossen um eins
...
// einziger direkt ueber `EINTRAEGE[43]` geprueft; die bisherige Reihe rueckt geschlossen um
...
// `EINTRAEGE[44]`, weil die Randregel der Spannachsen und die einheitliche Bezeichnung der
...
const EINBAUHOEHE = EINTRAEGE[44];
...
const EINBAULAGEN = EINTRAEGE[46];
...
const ZEICHNUNGSPDF = EINTRAEGE[47];
...
const VORSCHAUHOEHE = EINTRAEGE[48];
...
const STANDARDLAENGE = EINTRAEGE[49];
...
const KOPIERSCHUTZ = EINTRAEGE[50];
...
// zu drehen.
const VORHER = EINTRAEGE.slice(69);
const NEU = EINTRAEGE.slice(75);
const AKTUELL = EINTRAEGE.slice(80);
const AELTER = EINTRAEGE.slice(90);
...
const SPANNPLATTE = EINTRAEGE[52];
...
const WANDAUSWAHL = EINTRAEGE[53];
...
const VORRATSSATZ = EINTRAEGE[57];
...
const GESCHOSSMENGE = EINTRAEGE[59];
...
const TEILAUSWAHL = EINTRAEGE[60];
...
// unveraendert, es entsteht kein Feld und kein Versionssprung.
const IMPORTDIALOG = EINTRAEGE[61];
