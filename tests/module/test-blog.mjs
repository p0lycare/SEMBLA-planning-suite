...
// Der NEUESTE Eintrag ist der Spannplatten-Default (#92) — er wird als
// einziger direkt ueber `EINTRAEGE[0]` geprueft; die bisherige Reihe rueckt
// geschlossen um eins nach hinten und zaehlt ueber `WANDAUSWAHL`, `KOPFBLECH`,
...
// zu drehen.
const VORHER = EINTRAEGE.slice(17);
const NEU = EINTRAEGE.slice(23);
const AKTUELL = EINTRAEGE.slice(28);
const AELTER = EINTRAEGE.slice(38);
...
// Der NEUESTE Eintrag ist der Spannplatten-Default (#92) — er wird als einziger direkt
// ueber `EINTRAEGE[0]` geprueft. Aussagewahr heisst hier: versprochen wird die VORAUSWAHL
// fuer NEUE Waende und der Fortbestand einer gespeicherten Kopfblech-Wahl; nicht versprochen
// werden geaenderte Mengen, Preise, Katalogrollen oder eine Umstellung des Bestands.
ok("der Spannplatten-Default (Issue 92) ist der neueste Eintrag",
  EINTRAEGE[0]?.id === "chg-20260904-08" && EINTRAEGE[0]?.issue === 92
...
ok("der Eintrag benennt beide Seiten: neuer Standard und weiter waehlbares Kopfblech",
  /Spannplatte/.test(EINTRAEGE[0]?.titel || "")
  ...
  && !/Menge|Preis|Maßstab|neues Format|umgestellt|alle Wände/i.test(EINTRAEGE[0]?.titel || ""));
ok("die Testbitte benennt neue Wand UND Bestandswand als die beiden Faelle",
  /neue Wand/.test(EINTRAEGE[0]?.testbitte || "")
  && /Modul 1/.test(EINTRAEGE[0]?.testbitte || "")
  && /bestehende Wand/.test(EINTRAEGE[0]?.testbitte || "")
  && /Kopfblech/.test(EINTRAEGE[0]?.testbitte || ""));

// Die feste Reihenfolge der Wandauswahl (#101) rueckt geschlossen um eins nach hinten.
const WANDAUSWAHL = EINTRAEGE[1];
...
const GESCHOSSMENGE = EINTRAEGE[7];
...
// unveraendert, es entsteht kein Feld und kein Versionssprung.
const IMPORTDIALOG = EINTRAEGE[9];
