...
// Der NEUESTE Eintrag ist das eigene Katalogmodul (#108) — er wird als einziger direkt ueber
...
const KATALOGMODUL = EINTRAEGE[0];
ok("[#108] das eigene Katalogmodul ist der neueste Eintrag",
...
const PDFRUECKKEHR = EINTRAEGE[1];
...
const HANDBUCHREGELN = EINTRAEGE[4];
...
// einziges direkt ueber `EINTRAEGE[5]` geprueft; die bisherige Reihe rueckt geschlossen um eins
...
// einzige direkt ueber `EINTRAEGE[6]` geprueft. Aussagewahr heisst hier: geliefert ist
...
// `EINTRAEGE[10]`, weil die Randregel der Spannachsen und die einheitliche Bezeichnung der
...
const EINBAUHOEHE = EINTRAEGE[10];
...
const EINBAULAGEN = EINTRAEGE[12];
...
const ZEICHNUNGSPDF = EINTRAEGE[13];
...
const VORSCHAUHOEHE = EINTRAEGE[14];
...
const STANDARDLAENGE = EINTRAEGE[15];
...
const KOPIERSCHUTZ = EINTRAEGE[16];
...
// zu drehen.
const VORHER = EINTRAEGE.slice(35);
const NEU = EINTRAEGE.slice(41);
const AKTUELL = EINTRAEGE.slice(46);
const AELTER = EINTRAEGE.slice(56);
...
const SPANNPLATTE = EINTRAEGE[18];
...
const WANDAUSWAHL = EINTRAEGE[19];
...
const VORRATSSATZ = EINTRAEGE[23];
...
const GESCHOSSMENGE = EINTRAEGE[25];
...
const TEILAUSWAHL = EINTRAEGE[26];
...
// unveraendert, es entsteht kein Feld und kein Versionssprung.
const IMPORTDIALOG = EINTRAEGE[27];
