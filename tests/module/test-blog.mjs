...
// Die Aktualisierung von Modul 7 ohne Neuladen (#109) ist der neueste Eintrag — sie wird als
// einzige direkt ueber `EINTRAEGE[0]` geprueft; die bisherige Reihe rueckt geschlossen um eins
...
const ZEICHNUNGSSYNC = EINTRAEGE[0];
ok("[#109] die Aktualisierung von Modul 7 ist der neueste Eintrag",
...
// ueber `EINTRAEGE[1]` geprueft; die bisherige Reihe ist dafuer geschlossen um eins nach
...
const EDITIERMODUS = EINTRAEGE[1];
...
// direkt ueber `EINTRAEGE[2]` geprueft; die bisherige Reihe rueckt geschlossen um eins
...
const AUSGLEICHSMENGE = EINTRAEGE[2];
...
const MASSORIENTIERUNG = EINTRAEGE[3];
...
// als einzige direkt ueber `EINTRAEGE[4]` geprueft. Aussagewahr heisst hier: geliefert ist die
...
const AUSGLEICHSPUNKTE = EINTRAEGE[4];
...
const KATALOGMODUL = EINTRAEGE[5];
...
const PDFRUECKKEHR = EINTRAEGE[6];
...
const HANDBUCHREGELN = EINTRAEGE[9];
...
// einziges direkt ueber `EINTRAEGE[10]` geprueft; die bisherige Reihe rueckt geschlossen um eins
...
// einzige direkt ueber `EINTRAEGE[11]` geprueft. Aussagewahr heisst hier: geliefert ist
...
// einziger direkt ueber `EINTRAEGE[14]` geprueft; die bisherige Reihe rueckt geschlossen um
...
// `EINTRAEGE[15]`, weil die Randregel der Spannachsen und die einheitliche Bezeichnung der
...
const EINBAUHOEHE = EINTRAEGE[15];
...
const EINBAULAGEN = EINTRAEGE[17];
...
const ZEICHNUNGSPDF = EINTRAEGE[18];
...
const VORSCHAUHOEHE = EINTRAEGE[19];
...
const STANDARDLAENGE = EINTRAEGE[20];
...
const KOPIERSCHUTZ = EINTRAEGE[21];
...
// zu drehen.
const VORHER = EINTRAEGE.slice(40);
const NEU = EINTRAEGE.slice(46);
const AKTUELL = EINTRAEGE.slice(51);
const AELTER = EINTRAEGE.slice(61);
...
const SPANNPLATTE = EINTRAEGE[23];
...
const WANDAUSWAHL = EINTRAEGE[24];
...
const VORRATSSATZ = EINTRAEGE[28];
...
const GESCHOSSMENGE = EINTRAEGE[30];
...
const TEILAUSWAHL = EINTRAEGE[31];
...
// unveraendert, es entsteht kein Feld und kein Versionssprung.
const IMPORTDIALOG = EINTRAEGE[32];
