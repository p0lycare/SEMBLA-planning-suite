...
ok("das Duplizieren und Löschen im Geschosseditor (Issue 74) ist der neueste Eintrag",
  EINTRAEGE[0]?.id === "chg-20260813-07" && EINTRAEGE[0]?.issue === 74);
ok("der neue Editor-Eintrag benennt Kopie, Bestätigung und Rückgängig aussagewahr",
  /duplizieren/.test(EINTRAEGE[0]?.titel || "")
  && /Kopie/.test(EINTRAEGE[0]?.testbitte || "")
  && /fragt nach/.test(EINTRAEGE[0]?.testbitte || "")
  && /rückgängig/.test(EINTRAEGE[0]?.testbitte || ""));
ok("die textfreien Modulstarts 8 und 9 (Issue 72, drittes Paket) folgen direkt danach",
  EINTRAEGE[1]?.id === "chg-20260813-06" && EINTRAEGE[1]?.issue === 72);
ok("der Intro-Eintrag 8/9 benennt Modulbereich und unveränderte Funktionen aussagewahr",
  /Module 8 und 9/.test(EINTRAEGE[1]?.titel || "")
  && /Kopfleiste/.test(EINTRAEGE[1]?.testbitte || "")
  && /unverändert/.test(EINTRAEGE[1]?.testbitte || ""));
ok("die textfreien Modulstarts 5 bis 7 (Issue 72, zweites Paket) bleiben als dritter aktueller Eintrag erhalten",
  EINTRAEGE[2]?.id === "chg-20260813-05" && EINTRAEGE[2]?.issue === 72);
ok("der zweite Intro-Eintrag benennt Modulbereich und unveränderte Funktionen aussagewahr",
  /Module 5 bis 7/.test(EINTRAEGE[2]?.titel || "")
  && /Kopfleiste/.test(EINTRAEGE[2]?.testbitte || "")
  && /unverändert/.test(EINTRAEGE[2]?.testbitte || ""));
ok("die textfreien Modulstarts 1 bis 4 (Issue 72, erstes Paket) bleiben als vierter aktueller Eintrag erhalten",
  EINTRAEGE[3]?.id === "chg-20260813-04" && EINTRAEGE[3]?.issue === 72);
ok("der erste Intro-Eintrag benennt Modulbereich und unveränderte Funktionen aussagewahr",
  /Module 1 bis 4/.test(EINTRAEGE[3]?.titel || "")
  && /Kopfleiste/.test(EINTRAEGE[3]?.testbitte || "")
  && /unverändert/.test(EINTRAEGE[3]?.testbitte || ""));
ok("der Geschossplaner-Reiter 0,5 (Issue 43) bleibt als fünfter aktueller Eintrag erhalten",
  EINTRAEGE[4]?.id === "chg-20260813-03" && EINTRAEGE[4]?.issue === 43);
ok("der Reiter-Eintrag benennt Kopfleiste, aktives Geschoss und unveränderte Auswahl aussagewahr",
  /Reiter 0,5/.test(EINTRAEGE[4]?.titel || "")
  && /aktive Geschoss/.test(EINTRAEGE[4]?.testbitte || "")
  && /Auswahl/.test(EINTRAEGE[4]?.testbitte || ""));
ok("die Lageplan-Nummernblasen (Issue 73) bleiben als sechster aktueller Eintrag erhalten",
  EINTRAEGE[5]?.id === "chg-20260813-02" && EINTRAEGE[5]?.issue === 73);
ok("der Marker-Eintrag benennt Außenblase und entfallenen Vollständigkeitsblock aussagewahr",
  /Nummernblase/.test(EINTRAEGE[5]?.testbitte || "")
  && /Vollständigkeit/.test(EINTRAEGE[5]?.testbitte || "")
  && /entfallen/.test(EINTRAEGE[5]?.testbitte || ""));
ok("der hierarchische Export (Issue 67) bleibt als siebter aktueller Eintrag erhalten",
  EINTRAEGE[6]?.id === "chg-20260813-01" && EINTRAEGE[6]?.issue === 67);
ok("der Export-Eintrag benennt den Wegfall des Planbild-Transports aussagewahr",
  /Planbilder/.test(EINTRAEGE[6]?.testbitte || "") && /entfallen/.test(EINTRAEGE[6]?.testbitte || ""));
ok("die kollisionsfreien Maßzahlen (Issue 59) bleiben als achter aktueller Eintrag erhalten",
  EINTRAEGE[7]?.id === "chg-20260812-10" && EINTRAEGE[7]?.issue === 59);
ok("die textfreie Eingabespalte in Modul 1 (Issue 69, zweite Kürzung) bleibt als neunter aktueller Eintrag erhalten",
  EINTRAEGE[8]?.id === "chg-20260812-09" && EINTRAEGE[8]?.issue === 69);
ok("die fokussierte Projektanlage (Issue 68) bleibt als zehnter aktueller Eintrag erhalten",
  EINTRAEGE[9]?.id === "chg-20260812-08" && EINTRAEGE[9]?.issue === 68);
ok("die Wandbezeichnung in der Stückliste (Issue 70) bleibt als elfter aktueller Eintrag erhalten",
  EINTRAEGE[10]?.id === "chg-20260812-07" && EINTRAEGE[10]?.issue === 70);
ok("die kompakte Eingabespalte in Modul 1 (Issue 69, erste Kürzung) bleibt als zwölfter aktueller Eintrag erhalten",
  EINTRAEGE[11]?.id === "chg-20260812-06" && EINTRAEGE[11]?.issue === 69);
ok("die aktive Wand im Geschosseditor (Issue 66) bleibt als dreizehnter aktueller Eintrag erhalten",
  EINTRAEGE[12]?.id === "chg-20260812-05" && EINTRAEGE[12]?.issue === 66);
ok("die Blattreduktion zu Issue 61 bleibt als vierzehnter aktueller Eintrag erhalten",
  EINTRAEGE[13]?.id === "chg-20260812-04" && EINTRAEGE[13]?.issue === 61);
ok("der kompakte Lageplankopf zu Issue 59 bleibt als fünfzehnter aktueller Eintrag erhalten",
  EINTRAEGE[14]?.id === "chg-20260812-03" && EINTRAEGE[14]?.issue === 59);
