...
// Fachregelaenderung oder ein neues gespeichertes Feld.
// Der NEUESTE Eintrag ist die ZUGAENGLICHKEIT VON MODUL 4 (#144) — er wird als einziger
// direkt ueber `EINTRAEGE[0]` geprueft; die bisherige Reihe wird ueber die KENNUNG ihres
// Eintrags gesucht und rueckt deshalb geraeuschlos nach hinten.
//
// Aussagewahr heisst hier: geliefert sind benannte Bedienelemente und Positionsfelder, die
...
ok("[#145] die zugaengliche Bedienung von Modul 1 steht in der Reihe",
  AX_M1?.id === "chg-20260923-01" && AX_M1?.issue === 145
  && AX_M1?.typ === "feature" && AX_M1?.datum === "2026-09-23"
  && /Modul 1/.test(AX_M1?.titel || ""));
ok("[#145] der Modul-1-Eintrag benennt Benennung UND Dialogbedienung",
...
ok("[#145] die Modul-1-Testbitte nennt Tastaturweg und Fokusverhalten",
...
ok("[#136] die Ausgleichslage in Zeichnung und Montage steht als Eintrag davor",
  ...
  && /eigene oberste Reihe/.test(FRISCH_136F?.titel || ""));
ok("[#136] die Testbitte fuehrt den 2570-Bedienweg durch Modul 7 und Modul 5",
  /Modul 7/.test(FRISCH_136F?.testbitte || "")
  && /Modul 5/.test(FRISCH_136F?.testbitte || "")
  && /170 mm/.test(FRISCH_136F?.testbitte || ""));

// Davor die BEDIENUNG DER AUSGLEICHSLAGE (#136) — ueber die Kennung gesucht, weil
// sie nicht mehr der neueste Eintrag ist. Ihre Aussagen bleiben unveraendert: Modul 1
...
const FRISCH_136E = EINTRAEGE.find(e => e.id === "chg-20260917-05");
ok("[#136] die Bedienung der Ausgleichslage steht als Eintrag davor",
...
ok("[#124] der Titel benennt Randachsen und den 1125-mm-Hoechstabstand",
  ...
  && /1125 mm/.test(DECKE_124?.titel || "")
  && !/Statik|Nachweis|Preis|Norm|Format|Migration|speicher/i.test(DECKE_124?.titel || ""));
ok("[#124] die Testbitte fuehrt den echten Nutzerpfad in Modul 1",
  /Modul 1/.test(DECKE_124?.testbitte || "")
  && /rote Z/.test(DECKE_124?.testbitte || "")
  && /1125 mm/.test(DECKE_124?.testbitte || "")
  && /Spannachse/.test(DECKE_124?.testbitte || ""));
ok("[#124] die Testbitte verspricht keine Statik und kein neues Feld",
  !/Statik|Nachweis|Preis|Menge|Rechnung|Format|Migration|gespeichert wird|Option/i
    .test(DECKE_124?.testbitte || ""));

// Davor liegt die MASTABSGETREUE STANGENBREITE UND DER WEGFALL DER WANDUMRISSLINIE
// (#121/#123) — ueber die Kennung gesucht, weil sie nicht mehr der neueste Eintrag ist. Ihre
// Aussagen bleiben unveraendert.
...
const STANGE_121 = EINTRAEGE.find(e => e.id === "chg-20260910-10");
