...
// Die REALE KATALOGDICKE DER SPANNPLATTE (#97) ist der neueste Eintrag — er wird als
...
const PLATTENDICKE97 = EINTRAEGE[0];
...
// er wird als einziger direkt ueber `EINTRAEGE[1]` geprueft; die bisherige Reihe rueckt
...
const PDFNAMENSFOLGE107 = EINTRAEGE[1];
...
// als einziger direkt ueber `EINTRAEGE[2]` geprueft; die bisherige Reihe rueckt geschlossen
...
const STDKATALOG113 = EINTRAEGE[2];
...
// Eintrag — er wird als einziger direkt ueber `EINTRAEGE[3]` geprueft; die bisherige Reihe
...
const BESCHBLOCK113 = EINTRAEGE[3];
...
// zweitneueste Eintrag — er wird als einziger direkt ueber `EINTRAEGE[5]` geprueft; die bisherige
...
const BLECHSTOSS91 = EINTRAEGE[5];
...
// viertneueste Eintrag — er wird als einziger direkt ueber `EINTRAEGE[7]` geprueft; die
...
// `EINTRAEGE[8]` geprueft; die bisherige Reihe rueckt geschlossen um eins nach hinten.
...
const DECKENZEIGEN = EINTRAEGE[8];
...
// aus #94) ist der neueste Eintrag — er wird als einziger direkt ueber `EINTRAEGE[9]` geprueft;
...
// chg-20260907-20) ist der neueste Eintrag — er wird als einziger direkt ueber `EINTRAEGE[10]`
...
const SAMMELMEHRFACH = EINTRAEGE[10];
...
// einziger direkt ueber `EINTRAEGE[11]` geprueft; die bisherige Reihe rueckt geschlossen um eins
...
const OHNE_SCHEIBE = EINTRAEGE[11];
...
// mit [A-3]/#97) sind der neueste Eintrag — er wird als einziger direkt ueber `EINTRAEGE[12]`
...
const ANSICHTMASSTAB = EINTRAEGE[12];
...
// sie werden als einzige direkt ueber `EINTRAEGE[13]` geprueft; die bisherige Reihe rueckt
...
const SYMBOLMASSE = EINTRAEGE[13];
...
// er wird als einziger direkt ueber `EINTRAEGE[14]` geprueft; die bisherige Reihe rueckt geschlossen
// um eins nach hinten. Aussagewahr heisst hier: geliefert ist AUSSCHLIESSLICH, dass die
...
const SAMMELPRODUKTE = EINTRAEGE[14];
...
const SPANNSYMBOLE = EINTRAEGE[15];
...
// es wird als einziges direkt ueber `EINTRAEGE[16]` geprueft; die bisherige Reihe rueckt geschlossen
...
const SAMMELMERKMALE = EINTRAEGE[16];
...
const EINLEGEMENGE = EINTRAEGE[17];
...
// `EINTRAEGE[18]` geprueft; die bisherige Reihe rueckt geschlossen um eins
...
const ZEICHNUNGSSYNC = EINTRAEGE[18];
...
// ueber `EINTRAEGE[19]` geprueft; die bisherige Reihe ist dafuer geschlossen um eins nach
...
const EDITIERMODUS = EINTRAEGE[19];
...
// direkt ueber `EINTRAEGE[20]` geprueft; die bisherige Reihe rueckt geschlossen um eins
...
const AUSGLEICHSMENGE = EINTRAEGE[20];
...
// `EINTRAEGE[21]` geprueft; die bisherige Reihe rueckt geschlossen um eins nach hinten.
...
const MASSORIENTIERUNG = EINTRAEGE[21];
...
// als einzige direkt ueber `EINTRAEGE[22]` geprueft. Aussagewahr heisst hier: geliefert ist die
...
const AUSGLEICHSPUNKTE = EINTRAEGE[22];
...
const KATALOGMODUL = EINTRAEGE[23];
...
const PDFRUECKKEHR = EINTRAEGE[24];
...
const ACHSTREFFER = EINTRAEGE[25];
...
const HANDBUCHREGELN = EINTRAEGE[27];
...
// einziges direkt ueber `EINTRAEGE[28]` geprueft; die bisherige Reihe rueckt geschlossen um eins
...
// einzige direkt ueber `EINTRAEGE[29]` geprueft. Aussagewahr heisst hier: geliefert ist
...
// einziger direkt ueber `EINTRAEGE[30]` geprueft; die bisherige Reihe rueckt geschlossen um eins
...
// einziger direkt ueber `EINTRAEGE[32]` geprueft; die bisherige Reihe rueckt geschlossen um
...
// `EINTRAEGE[33]`, weil die Randregel der Spannachsen und die einheitliche Bezeichnung der
...
const EINBAUHOEHE = EINTRAEGE[33];
...
const EINBAULAGEN = EINTRAEGE[35];
...
const ZEICHNUNGSPDF = EINTRAEGE[36];
...
const VORSCHAUHOEHE = EINTRAEGE[37];
...
const STANDARDLAENGE = EINTRAEGE[38];
...
const KOPIERSCHUTZ = EINTRAEGE[39];
...
// zu drehen.
const VORHER = EINTRAEGE.slice(58);
const NEU = EINTRAEGE.slice(64);
const AKTUELL = EINTRAEGE.slice(69);
const AELTER = EINTRAEGE.slice(79);
...
const SPANNPLATTE = EINTRAEGE[41];
...
const WANDAUSWAHL = EINTRAEGE[42];
...
const VORRATSSATZ = EINTRAEGE[46];
...
const GESCHOSSMENGE = EINTRAEGE[48];
...
const TEILAUSWAHL = EINTRAEGE[49];
...
// unveraendert, es entsteht kein Feld und kein Versionssprung.
const IMPORTDIALOG = EINTRAEGE[50];
