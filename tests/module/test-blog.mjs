...
// Die BAUGRUPPEN-ANSICHT IN MODUL 4 (#94) ist der neueste Eintrag — er wird als einziger
...
const BAUGRUPPENANSICHT94 = EINTRAEGE[0];
ok("[#94] die Baugruppen-Ansicht in Modul 4 ist der neueste Eintrag",
...
const EINKAUFSLISTE113 = EINTRAEGE[1];
...
const PLATTENDICKE97 = EINTRAEGE[2];
...
ok("genau ein Eintrag fuer Issue 97 in diesem Paket",
  EINTRAEGE.filter(e => e.issue === 97 && e.datum === "2026-09-09").length === 1);

// Die NAMENSFOLGE DER WANDBLAETTER in der Zeichnungs-PDF (#107) ist der zweitneueste Eintrag —
// er wird als einziger direkt ueber `EINTRAEGE[3]` geprueft; die bisherige Reihe rueckt
...
const PDFNAMENSFOLGE107 = EINTRAEGE[3];
...
// als einziger direkt ueber `EINTRAEGE[4]` geprueft; die bisherige Reihe rueckt geschlossen
...
const STDKATALOG113 = EINTRAEGE[4];
...
// Eintrag — er wird als einziger direkt ueber `EINTRAEGE[5]` geprueft; die bisherige Reihe
...
const BESCHBLOCK113 = EINTRAEGE[5];
...
// er wird als einziger direkt ueber `EINTRAEGE[6]` geprueft; die bisherige Reihe rueckt
// geschlossen um eins nach hinten. Aussagewahr heisst hier: geliefert ist AUSSCHLIESSLICH,
...
// zweitneueste Eintrag — er wird als einziger direkt ueber `EINTRAEGE[7]` geprueft; die bisherige
...
const BLECHSTOSS91 = EINTRAEGE[7];
...
// viertneueste Eintrag — er wird als einziger direkt ueber `EINTRAEGE[9]` geprueft; die
...
// `EINTRAEGE[10]` geprueft; die bisherige Reihe rueckt geschlossen um eins nach hinten.
...
const DECKENZEIGEN = EINTRAEGE[10];
...
// aus #94) ist der neueste Eintrag — er wird als einziger direkt ueber `EINTRAEGE[11]` geprueft;
...
// chg-20260907-20) ist der neueste Eintrag — er wird als einziger direkt ueber `EINTRAEGE[12]`
...
const SAMMELMEHRFACH = EINTRAEGE[12];
...
// einziger direkt ueber `EINTRAEGE[13]` geprueft; die bisherige Reihe rueckt geschlossen um eins
...
const OHNE_SCHEIBE = EINTRAEGE[13];
...
// mit [A-3]/#97) sind der neueste Eintrag — er wird als einziger direkt ueber `EINTRAEGE[14]`
...
const ANSICHTMASSTAB = EINTRAEGE[14];
...
// sie werden als einzige direkt ueber `EINTRAEGE[15]` geprueft; die bisherige Reihe rueckt
...
const SYMBOLMASSE = EINTRAEGE[15];
...
// er wird als einziger direkt ueber `EINTRAEGE[16]` geprueft; die bisherige Reihe rueckt geschlossen
// um eins nach hinten. Aussagewahr heisst hier: geliefert ist AUSSCHLIESSLICH, dass die
...
const SAMMELPRODUKTE = EINTRAEGE[16];
...
// sie werden als einzige direkt ueber `EINTRAEGE[17]` geprueft; die bisherige Reihe rueckt
// geschlossen um eins nach hinten. Aussagewahr heisst hier: geliefert ist AUSSCHLIESSLICH, dass
...
const SPANNSYMBOLE = EINTRAEGE[17];
...
// es wird als einziges direkt ueber `EINTRAEGE[18]` geprueft; die bisherige Reihe rueckt geschlossen
...
const SAMMELMERKMALE = EINTRAEGE[18];
...
// sie werden als einzige direkt ueber `EINTRAEGE[19]` geprueft; die bisherige Reihe rueckt
// geschlossen um eins nach hinten. Aussagewahr heisst hier: geliefert ist AUSSCHLIESSLICH, dass
...
const EINLEGEMENGE = EINTRAEGE[19];
...
// `EINTRAEGE[20]` geprueft; die bisherige Reihe rueckt geschlossen um eins
...
const ZEICHNUNGSSYNC = EINTRAEGE[20];
...
// ueber `EINTRAEGE[21]` geprueft; die bisherige Reihe ist dafuer geschlossen um eins nach
...
const EDITIERMODUS = EINTRAEGE[21];
...
// direkt ueber `EINTRAEGE[22]` geprueft; die bisherige Reihe rueckt geschlossen um eins
...
const AUSGLEICHSMENGE = EINTRAEGE[22];
...
// `EINTRAEGE[23]` geprueft; die bisherige Reihe rueckt geschlossen um eins nach hinten.
...
const MASSORIENTIERUNG = EINTRAEGE[23];
...
// als einzige direkt ueber `EINTRAEGE[24]` geprueft. Aussagewahr heisst hier: geliefert ist die
...
const AUSGLEICHSPUNKTE = EINTRAEGE[24];
...
const KATALOGMODUL = EINTRAEGE[25];
...
const PDFRUECKKEHR = EINTRAEGE[26];
...
const ACHSTREFFER = EINTRAEGE[27];
...
const HANDBUCHREGELN = EINTRAEGE[29];
...
// einziges direkt ueber `EINTRAEGE[30]` geprueft; die bisherige Reihe rueckt geschlossen um eins
...
// einzige direkt ueber `EINTRAEGE[31]` geprueft. Aussagewahr heisst hier: geliefert ist
...
// einziger direkt ueber `EINTRAEGE[32]` geprueft; die bisherige Reihe rueckt geschlossen um eins
...
// einziger direkt ueber `EINTRAEGE[34]` geprueft; die bisherige Reihe rueckt geschlossen um
...
// `EINTRAEGE[35]`, weil die Randregel der Spannachsen und die einheitliche Bezeichnung der
...
const EINBAUHOEHE = EINTRAEGE[35];
...
const EINBAULAGEN = EINTRAEGE[37];
...
const ZEICHNUNGSPDF = EINTRAEGE[38];
...
const VORSCHAUHOEHE = EINTRAEGE[39];
...
const STANDARDLAENGE = EINTRAEGE[40];
...
const KOPIERSCHUTZ = EINTRAEGE[41];
...
// zu drehen.
const VORHER = EINTRAEGE.slice(60);
const NEU = EINTRAEGE.slice(66);
const AKTUELL = EINTRAEGE.slice(71);
const AELTER = EINTRAEGE.slice(81);
...
const SPANNPLATTE = EINTRAEGE[43];
...
const WANDAUSWAHL = EINTRAEGE[44];
...
const VORRATSSATZ = EINTRAEGE[48];
...
const GESCHOSSMENGE = EINTRAEGE[50];
...
const TEILAUSWAHL = EINTRAEGE[51];
...
// unveraendert, es entsteht kein Feld und kein Versionssprung.
const IMPORTDIALOG = EINTRAEGE[52];
