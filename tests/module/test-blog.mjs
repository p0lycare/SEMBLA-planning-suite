...
// Die AUSDRUECKLICHE RUECKFRAGE vor der Katalogvariante (#108) ist der neueste Eintrag —
...
// zweitneueste Eintrag — er wird als einziger direkt ueber `EINTRAEGE[1]` geprueft; die bisherige
...
const BLECHSTOSS91 = EINTRAEGE[1];
...
// viertneueste Eintrag — er wird als einziger direkt ueber `EINTRAEGE[3]` geprueft; die
...
// `EINTRAEGE[4]` geprueft; die bisherige Reihe rueckt geschlossen um eins nach hinten.
...
const DECKENZEIGEN = EINTRAEGE[4];
...
// aus #94) ist der neueste Eintrag — er wird als einziger direkt ueber `EINTRAEGE[5]` geprueft;
...
// chg-20260907-20) ist der neueste Eintrag — er wird als einziger direkt ueber `EINTRAEGE[6]`
...
const SAMMELMEHRFACH = EINTRAEGE[6];
...
const OHNE_SCHEIBE = EINTRAEGE[7];
...
// mit [A-3]/#97) sind der neueste Eintrag — er wird als einziger direkt ueber `EINTRAEGE[8]`
...
const ANSICHTMASSTAB = EINTRAEGE[8];
...
// er wird als einziger direkt ueber `EINTRAEGE[10]` geprueft; die bisherige Reihe rueckt geschlossen
...
const SAMMELPRODUKTE = EINTRAEGE[10];
...
const SPANNSYMBOLE = EINTRAEGE[11];
...
// es wird als einziges direkt ueber `EINTRAEGE[12]` geprueft; die bisherige Reihe rueckt geschlossen
...
const SAMMELMERKMALE = EINTRAEGE[12];
...
const EINLEGEMENGE = EINTRAEGE[13];
...
// `EINTRAEGE[14]` geprueft; die bisherige Reihe rueckt geschlossen um eins
...
const ZEICHNUNGSSYNC = EINTRAEGE[14];
...
// ueber `EINTRAEGE[15]` geprueft; die bisherige Reihe ist dafuer geschlossen um eins nach
...
const EDITIERMODUS = EINTRAEGE[15];
...
// direkt ueber `EINTRAEGE[16]` geprueft; die bisherige Reihe rueckt geschlossen um eins
...
const AUSGLEICHSMENGE = EINTRAEGE[16];
...
const MASSORIENTIERUNG = EINTRAEGE[17];
...
// als einzige direkt ueber `EINTRAEGE[18]` geprueft. Aussagewahr heisst hier: geliefert ist die
...
const AUSGLEICHSPUNKTE = EINTRAEGE[18];
...
const KATALOGMODUL = EINTRAEGE[19];
...
const PDFRUECKKEHR = EINTRAEGE[20];
...
const ACHSTREFFER = EINTRAEGE[21];
...
const HANDBUCHREGELN = EINTRAEGE[23];
...
// einziges direkt ueber `EINTRAEGE[24]` geprueft; die bisherige Reihe rueckt geschlossen um eins
...
// einzige direkt ueber `EINTRAEGE[25]` geprueft. Aussagewahr heisst hier: geliefert ist
...
// einziger direkt ueber `EINTRAEGE[26]` geprueft; die bisherige Reihe rueckt geschlossen um eins
...
// einziger direkt ueber `EINTRAEGE[28]` geprueft; die bisherige Reihe rueckt geschlossen um
...
// `EINTRAEGE[29]`, weil die Randregel der Spannachsen und die einheitliche Bezeichnung der
...
const EINBAUHOEHE = EINTRAEGE[29];
...
const EINBAULAGEN = EINTRAEGE[31];
...
const ZEICHNUNGSPDF = EINTRAEGE[32];
...
const VORSCHAUHOEHE = EINTRAEGE[33];
...
const STANDARDLAENGE = EINTRAEGE[34];
...
const KOPIERSCHUTZ = EINTRAEGE[35];
...
// zu drehen.
const VORHER = EINTRAEGE.slice(54);
const NEU = EINTRAEGE.slice(60);
const AKTUELL = EINTRAEGE.slice(65);
const AELTER = EINTRAEGE.slice(75);
...
const SPANNPLATTE = EINTRAEGE[37];
...
const WANDAUSWAHL = EINTRAEGE[38];
...
const VORRATSSATZ = EINTRAEGE[42];
...
const GESCHOSSMENGE = EINTRAEGE[44];
...
const TEILAUSWAHL = EINTRAEGE[45];
...
// unveraendert, es entsteht kein Feld und kein Versionssprung.
const IMPORTDIALOG = EINTRAEGE[46];
