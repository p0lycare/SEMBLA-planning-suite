...
// Der NEUESTE Eintrag sind die deterministisch berechneten Ausgleichspunkte (#96) — er wird
// als einziger direkt ueber `EINTRAEGE[0]` geprueft; die bisherige Reihe rueckt geschlossen um
// eins nach hinten. Aussagewahr heisst hier: geliefert ist die deterministische VERTEILUNG der
// Punkte im Rechenkern und im Python-Orakel ([A-20]…[A-23]) — Dichte drei je Meter mit den
...
const AUSGLEICHSPUNKTE = EINTRAEGE[0];
...
const KATALOGMODUL = EINTRAEGE[1];
...
const PDFRUECKKEHR = EINTRAEGE[2];
...
const HANDBUCHREGELN = EINTRAEGE[5];
...
// einziges direkt ueber `EINTRAEGE[6]` geprueft; die bisherige Reihe rueckt geschlossen um eins
...
// einzige direkt ueber `EINTRAEGE[7]` geprueft. Aussagewahr heisst hier: geliefert ist
...
// `EINTRAEGE[11]`, weil die Randregel der Spannachsen und die einheitliche Bezeichnung der
...
const EINBAUHOEHE = EINTRAEGE[11];
...
const EINBAULAGEN = EINTRAEGE[13];
...
const ZEICHNUNGSPDF = EINTRAEGE[14];
...
const VORSCHAUHOEHE = EINTRAEGE[15];
...
const STANDARDLAENGE = EINTRAEGE[16];
...
const KOPIERSCHUTZ = EINTRAEGE[17];
...
// zu drehen.
const VORHER = EINTRAEGE.slice(36);
const NEU = EINTRAEGE.slice(42);
const AKTUELL = EINTRAEGE.slice(47);
const AELTER = EINTRAEGE.slice(57);
...
const SPANNPLATTE = EINTRAEGE[19];
...
const WANDAUSWAHL = EINTRAEGE[20];
...
const VORRATSSATZ = EINTRAEGE[24];
...
const GESCHOSSMENGE = EINTRAEGE[26];
...
const TEILAUSWAHL = EINTRAEGE[27];
...
// unveraendert, es entsteht kein Feld und kein Versionssprung.
const IMPORTDIALOG = EINTRAEGE[28];
