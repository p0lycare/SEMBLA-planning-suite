...
// Die NACHGEZOGENE SCHLUESSELWEITE der Sammelaenderung (#97) ist der neueste Eintrag — er
...
const SAMMEL_SW97 = EINTRAEGE[0];
ok("[#97] die nachgezogene Schluesselweite der Sammelaenderung ist der neueste Eintrag",
...
// direkt ueber `EINTRAEGE[3]` geprueft; die bisherige Reihe rueckt geschlossen um eins nach
...
const SCHLUESSELWEITE97 = EINTRAEGE[3];
...
// als einziger direkt ueber `EINTRAEGE[4]` geprueft; die bisherige Reihe rueckt geschlossen um
...
const NEURECHNUNG117 = EINTRAEGE[4];
...
// als einziger direkt ueber `EINTRAEGE[5]` geprueft; die bisherige Reihe rueckt geschlossen um
...
const FEHLREFERENZ116 = EINTRAEGE[5];
...
// einziger direkt ueber `EINTRAEGE[6]` geprueft; die bisherige Reihe rueckt geschlossen um
...
const STANGENVORKOPPLUNG112 = EINTRAEGE[6];
...
// wird als einziger direkt ueber `EINTRAEGE[7]` geprueft; die bisherige Reihe rueckt
// geschlossen um eins nach hinten. Aussagewahr heisst hier: geliefert ist AUSSCHLIESSLICH,
...
const ALTBESTAND114 = EINTRAEGE[7];
...
const EBENENNACHWEIS94 = EINTRAEGE[8];
...
// einziger direkt ueber `EINTRAEGE[9]` geprueft; die bisherige Reihe rueckt geschlossen um eins
...
const MUTTERNHOEHE97 = EINTRAEGE[9];
...
// direkt ueber `EINTRAEGE[10]` geprueft; die bisherige Reihe rueckt geschlossen um eins nach
...
const BAUGRUPPENANSICHT94 = EINTRAEGE[10];
...
// als einziger direkt ueber `EINTRAEGE[11]` geprueft; die bisherige Reihe rueckt geschlossen um
...
const EINKAUFSLISTE113 = EINTRAEGE[11];
...
// einziger direkt ueber `EINTRAEGE[12]` geprueft; die bisherige Reihe rueckt geschlossen um
...
const PLATTENDICKE97 = EINTRAEGE[12];
...
// (chg-20260909-17). Je Paket GENAU EIN Eintrag — und keiner mehr, damit dieselbe Aenderung
// nicht zweimal in der Liste steht. Die vier letzten sind ausdruecklich VIER Pakete: das
// erste pflegt das Mass im Katalog, das zweite fuehrt es an die Wand, das dritte zeichnet es,
// das vierte schliesst die zweite Schreibbahn im Geschosseditor.
...
// er wird als einziger direkt ueber `EINTRAEGE[13]` geprueft; die bisherige Reihe rueckt
// geschlossen um eins nach hinten. Aussagewahr heisst hier: geliefert ist AUSSCHLIESSLICH,
...
const PDFNAMENSFOLGE107 = EINTRAEGE[13];
...
// als einziger direkt ueber `EINTRAEGE[14]` geprueft; die bisherige Reihe rueckt geschlossen
...
const STDKATALOG113 = EINTRAEGE[14];
...
// Eintrag — er wird als einziger direkt ueber `EINTRAEGE[15]` geprueft; die bisherige Reihe
...
const BESCHBLOCK113 = EINTRAEGE[15];
...
const VARIANTE108 = EINTRAEGE[16];
...
// zweitneueste Eintrag — er wird als einziger direkt ueber `EINTRAEGE[17]` geprueft; die bisherige
...
const BLECHSTOSS91 = EINTRAEGE[17];
...
// er wird als einziger direkt ueber `EINTRAEGE[18]` geprueft; die bisherige Reihe rueckt
// geschlossen um eins nach hinten. Aussagewahr heisst hier: geliefert ist AUSSCHLIESSLICH,
...
const BESCHAFFUNG = EINTRAEGE[18];
...
// viertneueste Eintrag — er wird als einziger direkt ueber `EINTRAEGE[19]` geprueft; die
...
const STANGENVORN = EINTRAEGE[19];
...
// `EINTRAEGE[20]` geprueft; die bisherige Reihe rueckt geschlossen um eins nach hinten.
...
const DECKENZEIGEN = EINTRAEGE[20];
...
// aus #94) ist der neueste Eintrag — er wird als einziger direkt ueber `EINTRAEGE[21]` geprueft;
...
// chg-20260907-20) ist der neueste Eintrag — er wird als einziger direkt ueber `EINTRAEGE[22]`
...
const SAMMELMEHRFACH = EINTRAEGE[22];
...
const OHNE_SCHEIBE = EINTRAEGE[23];
...
// mit [A-3]/#97) sind der neueste Eintrag — er wird als einziger direkt ueber `EINTRAEGE[24]`
...
const ANSICHTMASSTAB = EINTRAEGE[24];
...
const SYMBOLMASSE = EINTRAEGE[25];
...
// er wird als einziger direkt ueber `EINTRAEGE[26]` geprueft; die bisherige Reihe rueckt geschlossen
// um eins nach hinten. Aussagewahr heisst hier: geliefert ist AUSSCHLIESSLICH, dass die
...
const SAMMELPRODUKTE = EINTRAEGE[26];
...
const SPANNSYMBOLE = EINTRAEGE[27];
...
// es wird als einziges direkt ueber `EINTRAEGE[28]` geprueft; die bisherige Reihe rueckt geschlossen
...
const SAMMELMERKMALE = EINTRAEGE[28];
...
// sie werden als einzige direkt ueber `EINTRAEGE[29]` geprueft; die bisherige Reihe rueckt
// geschlossen um eins nach hinten. Aussagewahr heisst hier: geliefert ist AUSSCHLIESSLICH, dass
...
const EINLEGEMENGE = EINTRAEGE[29];
...
// `EINTRAEGE[30]` geprueft; die bisherige Reihe rueckt geschlossen um eins
...
const ZEICHNUNGSSYNC = EINTRAEGE[30];
...
// ueber `EINTRAEGE[31]` geprueft; die bisherige Reihe ist dafuer geschlossen um eins nach
...
const EDITIERMODUS = EINTRAEGE[31];
...
// direkt ueber `EINTRAEGE[32]` geprueft; die bisherige Reihe rueckt geschlossen um eins
...
const AUSGLEICHSMENGE = EINTRAEGE[32];
...
// `EINTRAEGE[33]` geprueft; die bisherige Reihe rueckt geschlossen um eins nach hinten.
...
const MASSORIENTIERUNG = EINTRAEGE[33];
...
// als einzige direkt ueber `EINTRAEGE[34]` geprueft. Aussagewahr heisst hier: geliefert ist die
...
const AUSGLEICHSPUNKTE = EINTRAEGE[34];
...
const KATALOGMODUL = EINTRAEGE[35];
...
const PDFRUECKKEHR = EINTRAEGE[36];
...
const ACHSTREFFER = EINTRAEGE[37];
...
const HANDBUCHREGELN = EINTRAEGE[39];
...
// einziges direkt ueber `EINTRAEGE[40]` geprueft; die bisherige Reihe rueckt geschlossen um eins
...
// einzige direkt ueber `EINTRAEGE[41]` geprueft. Aussagewahr heisst hier: geliefert ist
...
// einziger direkt ueber `EINTRAEGE[42]` geprueft; die bisherige Reihe rueckt geschlossen um eins
...
// einziger direkt ueber `EINTRAEGE[44]` geprueft; die bisherige Reihe rueckt geschlossen um
...
// `EINTRAEGE[45]`, weil die Randregel der Spannachsen und die einheitliche Bezeichnung der
...
const EINBAUHOEHE = EINTRAEGE[45];
...
const EINBAULAGEN = EINTRAEGE[47];
...
const ZEICHNUNGSPDF = EINTRAEGE[48];
...
const VORSCHAUHOEHE = EINTRAEGE[49];
...
const STANDARDLAENGE = EINTRAEGE[50];
...
const KOPIERSCHUTZ = EINTRAEGE[51];
...
// zu drehen.
const VORHER = EINTRAEGE.slice(70);
const NEU = EINTRAEGE.slice(76);
const AKTUELL = EINTRAEGE.slice(81);
const AELTER = EINTRAEGE.slice(91);
...
const SPANNPLATTE = EINTRAEGE[53];
...
const WANDAUSWAHL = EINTRAEGE[54];
...
const VORRATSSATZ = EINTRAEGE[58];
...
const GESCHOSSMENGE = EINTRAEGE[60];
...
const TEILAUSWAHL = EINTRAEGE[61];
...
// unveraendert, es entsteht kein Feld und kein Versionssprung.
const IMPORTDIALOG = EINTRAEGE[62];
