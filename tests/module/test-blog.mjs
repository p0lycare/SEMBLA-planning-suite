...
// Die FESTEN Bauteilsymbole und die Fussfolge (#112, mit [A-19]/#97) sind der neueste Eintrag —
// sie werden als einzige direkt ueber `EINTRAEGE[0]` geprueft; die bisherige Reihe rueckt
...
const SYMBOLMASSE = EINTRAEGE[0];
ok("[#112] die festen Bauteilsymbole sind der neueste Eintrag",
  SYMBOLMASSE?.id === "chg-20260908-22" && SYMBOLMASSE?.issue === 112
  && SYMBOLMASSE?.typ === "fix" && SYMBOLMASSE?.datum === "2026-09-08");
ok("[#112] der Eintrag benennt beide Gegenstaende: gleiche Groesse und die Fussfolge",
...
ok("[#112] die Testbitte fuehrt den echten Nutzerpfad ueber beide Ansichten",
  /Modul 1/.test(SYMBOLMASSE?.testbitte || "")
  && /Modul 7/.test(SYMBOLMASSE?.testbitte || "")
  && /verschiedener Länge/.test(SYMBOLMASSE?.testbitte || ""));
ok("[#112] die Testbitte benennt den ALTEN Zustand, an dem man die Korrektur erkennt",
  /vorher/.test(SYMBOLMASSE?.testbitte || "")
  && /Mehrfaches größer/.test(SYMBOLMASSE?.testbitte || "")
  && /halb im Blech/.test(SYMBOLMASSE?.testbitte || ""));
ok("[#112] die Testbitte benennt die vollstaendige Fussfolge und die obere Spannplatte",
...
ok("[#112] die Testbitte verspricht nichts, was dieser Stand nicht liefert",
  !/Menge|Stückliste|Preis|Bemaßung|Maßzahl|Nachweis|Statik|Montage|Migration/i
    .test(SYMBOLMASSE?.testbitte || ""));
ok("genau ein Eintrag fuer Issue 112", EINTRAEGE.filter(e => e.issue === 112).length === 1);

// Die gemeinsame PRODUKTAUSWAHL im Sammel-Popup (#111, Folgepaket) ist der neueste Eintrag —
// er wird als einziger direkt ueber `EINTRAEGE[1]` geprueft; die bisherige Reihe rueckt geschlossen
...
const SAMMELPRODUKTE = EINTRAEGE[1];
...
// sie werden als einzige direkt ueber `EINTRAEGE[2]` geprueft; die bisherige Reihe rueckt
// geschlossen um eins nach hinten. Aussagewahr heisst hier: geliefert ist AUSSCHLIESSLICH, dass
...
const SPANNSYMBOLE = EINTRAEGE[2];
...
// es wird als einziges direkt ueber `EINTRAEGE[3]` geprueft; die bisherige Reihe rueckt geschlossen
...
const SAMMELMERKMALE = EINTRAEGE[3];
...
// Zwei getrennte Pakete zu #111 — Wandmerkmale und Produktauswahl —, also zwei Commits und
// zwei Eintraege, in dieser Reihenfolge (neu vor alt).
ok("genau zwei Eintraege fuer Issue 111, neu vor alt",
  EINTRAEGE.filter(e => e.issue === 111).map(e => e.id).join(",")
    === "chg-20260907-20,chg-20260907-18");

// Die Stuecklistenpositionen fuer Einlegeblech und Mutter (#93/#109) sind der neueste Eintrag —
// sie werden als einzige direkt ueber `EINTRAEGE[4]` geprueft; die bisherige Reihe rueckt
// geschlossen um eins nach hinten. Aussagewahr heisst hier: geliefert ist AUSSCHLIESSLICH, dass
...
const EINLEGEMENGE = EINTRAEGE[4];
...
// `EINTRAEGE[5]` geprueft; die bisherige Reihe rueckt geschlossen um eins
...
const ZEICHNUNGSSYNC = EINTRAEGE[5];
...
// ueber `EINTRAEGE[6]` geprueft; die bisherige Reihe ist dafuer geschlossen um eins nach
...
const EDITIERMODUS = EINTRAEGE[6];
...
// direkt ueber `EINTRAEGE[7]` geprueft; die bisherige Reihe rueckt geschlossen um eins
...
const AUSGLEICHSMENGE = EINTRAEGE[7];
...
const MASSORIENTIERUNG = EINTRAEGE[8];
...
// als einzige direkt ueber `EINTRAEGE[9]` geprueft. Aussagewahr heisst hier: geliefert ist die
...
const AUSGLEICHSPUNKTE = EINTRAEGE[9];
...
const KATALOGMODUL = EINTRAEGE[10];
...
const PDFRUECKKEHR = EINTRAEGE[11];
...
const ACHSTREFFER = EINTRAEGE[12];
...
ok("genau ein Eintrag fuer Issue 106 (Bedienteil)",
  EINTRAEGE.filter(e => e.issue === 106).length === 1);
...
const HANDBUCHREGELN = EINTRAEGE[14];
...
// einziges direkt ueber `EINTRAEGE[15]` geprueft; die bisherige Reihe rueckt geschlossen um eins
...
// einzige direkt ueber `EINTRAEGE[16]` geprueft. Aussagewahr heisst hier: geliefert ist
...
// einziger direkt ueber `EINTRAEGE[19]` geprueft; die bisherige Reihe rueckt geschlossen um
...
// `EINTRAEGE[20]`, weil die Randregel der Spannachsen und die einheitliche Bezeichnung der
...
const EINBAUHOEHE = EINTRAEGE[20];
...
const EINBAULAGEN = EINTRAEGE[22];
...
const ZEICHNUNGSPDF = EINTRAEGE[23];
...
const VORSCHAUHOEHE = EINTRAEGE[24];
...
const STANDARDLAENGE = EINTRAEGE[25];
...
const KOPIERSCHUTZ = EINTRAEGE[26];
...
// zu drehen.
const VORHER = EINTRAEGE.slice(45);
const NEU = EINTRAEGE.slice(51);
const AKTUELL = EINTRAEGE.slice(56);
const AELTER = EINTRAEGE.slice(66);
...
const SPANNPLATTE = EINTRAEGE[28];
...
const WANDAUSWAHL = EINTRAEGE[29];
...
const VORRATSSATZ = EINTRAEGE[33];
...
const GESCHOSSMENGE = EINTRAEGE[35];
...
const TEILAUSWAHL = EINTRAEGE[36];
...
// unveraendert, es entsteht kein Feld und kein Versionssprung.
const IMPORTDIALOG = EINTRAEGE[37];
