...
// Der NEUESTE Eintrag sind die gesammelten Zeichnungs-PDFs (#98) — er wird als einziger
// direkt ueber `EINTRAEGE[0]` geprueft; die bisherige Reihe rueckt geschlossen um eins nach
// hinten. Aussagewahr heisst hier: versprochen wird GENAU der eine ZIP-Download mit einer PDF
...
ok("[#98] die gesammelten Zeichnungs-PDFs sind der neueste Eintrag",
  EINTRAEGE[0]?.id === "chg-20260905-01" && EINTRAEGE[0]?.issue === 98
  && EINTRAEGE[0]?.typ === "feature" && EINTRAEGE[0]?.datum === "2026-09-05");
ok("[#98] der Eintrag benennt Gegenstand, Umfang und Nutzerergebnis",
  /Zeichnungen/.test(EINTRAEGE[0]?.titel || "")
  && /geschossweise/i.test(EINTRAEGE[0]?.titel || "")
  && /PDF/.test(EINTRAEGE[0]?.titel || "")
  && /ZIP/.test(EINTRAEGE[0]?.titel || "")
...
ok("[#98] die Testbitte fuehrt den Bedienweg, die Seitenfolge und den Fehlerpfad",
  ...
  && /Zeichnungen als PDF/.test(EINTRAEGE[0]?.testbitte || "")
  && /je Geschoss/.test(EINTRAEGE[0]?.testbitte || "")
  && /Seite 1/.test(EINTRAEGE[0]?.testbitte || "")
  && /Wandelement/.test(EINTRAEGE[0]?.testbitte || ""));
...
const VORSCHAUHOEHE = EINTRAEGE[1];
...
const STANDARDLAENGE = EINTRAEGE[2];
...
const KOPIERSCHUTZ = EINTRAEGE[3];
...
// zu drehen.
const VORHER = EINTRAEGE.slice(22);
const NEU = EINTRAEGE.slice(28);
const AKTUELL = EINTRAEGE.slice(33);
const AELTER = EINTRAEGE.slice(43);
...
const VORRATSSATZ = EINTRAEGE[10];
...
const GESCHOSSMENGE = EINTRAEGE[12];
...
const TEILAUSWAHL = EINTRAEGE[13];
...
// unveraendert, es entsteht kein Feld und kein Versionssprung.
const IMPORTDIALOG = EINTRAEGE[14];
