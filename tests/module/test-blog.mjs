...
// Der NEUESTE Eintrag ist die korrigierte Maßorientierung des vorlaeufigen Ausgleichsblechs
...
const MASSORIENTIERUNG = EINTRAEGE[0];
...
// als einzige direkt ueber `EINTRAEGE[1]` geprueft. Aussagewahr heisst hier: geliefert ist die
...
const AUSGLEICHSPUNKTE = EINTRAEGE[1];
...
const KATALOGMODUL = EINTRAEGE[2];
...
const PDFRUECKKEHR = EINTRAEGE[3];
...
const HANDBUCHREGELN = EINTRAEGE[6];
...
// einziges direkt ueber `EINTRAEGE[7]` geprueft; die bisherige Reihe rueckt geschlossen um eins
...
// einzige direkt ueber `EINTRAEGE[8]` geprueft. Aussagewahr heisst hier: geliefert ist
...
// `EINTRAEGE[12]`, weil die Randregel der Spannachsen und die einheitliche Bezeichnung der
...
const EINBAUHOEHE = EINTRAEGE[12];
...
const EINBAULAGEN = EINTRAEGE[14];
...
const ZEICHNUNGSPDF = EINTRAEGE[15];
...
const VORSCHAUHOEHE = EINTRAEGE[16];
...
const STANDARDLAENGE = EINTRAEGE[17];
...
const KOPIERSCHUTZ = EINTRAEGE[18];
...
// zu drehen.
const VORHER = EINTRAEGE.slice(37);
const NEU = EINTRAEGE.slice(43);
const AKTUELL = EINTRAEGE.slice(48);
const AELTER = EINTRAEGE.slice(58);
...
const SPANNPLATTE = EINTRAEGE[20];
...
const WANDAUSWAHL = EINTRAEGE[21];
...
const VORRATSSATZ = EINTRAEGE[25];
...
const GESCHOSSMENGE = EINTRAEGE[27];
...
const TEILAUSWAHL = EINTRAEGE[28];
...
// unveraendert, es entsteht kein Feld und kein Versionssprung.
const IMPORTDIALOG = EINTRAEGE[29];
