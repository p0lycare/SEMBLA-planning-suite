...
const OHNE_SCHEIBE = EINTRAEGE[0];
...
// mit [A-3]/#97) sind der neueste Eintrag — er wird als einziger direkt ueber `EINTRAEGE[1]`
...
const ANSICHTMASSTAB = EINTRAEGE[1];
...
// er wird als einziger direkt ueber `EINTRAEGE[3]` geprueft; die bisherige Reihe rueckt geschlossen
...
const SAMMELPRODUKTE = EINTRAEGE[3];
...
const SPANNSYMBOLE = EINTRAEGE[4];
...
// es wird als einziges direkt ueber `EINTRAEGE[5]` geprueft; die bisherige Reihe rueckt geschlossen
...
const SAMMELMERKMALE = EINTRAEGE[5];
...
const EINLEGEMENGE = EINTRAEGE[6];
...
// `EINTRAEGE[7]` geprueft; die bisherige Reihe rueckt geschlossen um eins
...
const ZEICHNUNGSSYNC = EINTRAEGE[7];
...
// ueber `EINTRAEGE[8]` geprueft; die bisherige Reihe ist dafuer geschlossen um eins nach
...
const EDITIERMODUS = EINTRAEGE[8];
...
// direkt ueber `EINTRAEGE[9]` geprueft; die bisherige Reihe rueckt geschlossen um eins
...
const AUSGLEICHSMENGE = EINTRAEGE[9];
...
const MASSORIENTIERUNG = EINTRAEGE[10];
...
// als einzige direkt ueber `EINTRAEGE[11]` geprueft. Aussagewahr heisst hier: geliefert ist die
...
const AUSGLEICHSPUNKTE = EINTRAEGE[11];
...
const KATALOGMODUL = EINTRAEGE[12];
...
const PDFRUECKKEHR = EINTRAEGE[13];
...
const ACHSTREFFER = EINTRAEGE[14];
...
const HANDBUCHREGELN = EINTRAEGE[16];
...
// einziges direkt ueber `EINTRAEGE[17]` geprueft; die bisherige Reihe rueckt geschlossen um eins
...
// einzige direkt ueber `EINTRAEGE[18]` geprueft. Aussagewahr heisst hier: geliefert ist
...
// einziger direkt ueber `EINTRAEGE[21]` geprueft; die bisherige Reihe rueckt geschlossen um
...
// `EINTRAEGE[22]`, weil die Randregel der Spannachsen und die einheitliche Bezeichnung der
...
const EINBAUHOEHE = EINTRAEGE[22];
...
const EINBAULAGEN = EINTRAEGE[24];
...
const ZEICHNUNGSPDF = EINTRAEGE[25];
...
const VORSCHAUHOEHE = EINTRAEGE[26];
...
const STANDARDLAENGE = EINTRAEGE[27];
...
const KOPIERSCHUTZ = EINTRAEGE[28];
...
// zu drehen.
const VORHER = EINTRAEGE.slice(47);
const NEU = EINTRAEGE.slice(53);
const AKTUELL = EINTRAEGE.slice(58);
const AELTER = EINTRAEGE.slice(68);
...
const SPANNPLATTE = EINTRAEGE[30];
...
const WANDAUSWAHL = EINTRAEGE[31];
...
const VORRATSSATZ = EINTRAEGE[35];
...
const GESCHOSSMENGE = EINTRAEGE[37];
...
const TEILAUSWAHL = EINTRAEGE[38];
...
// unveraendert, es entsteht kein Feld und kein Versionssprung.
const IMPORTDIALOG = EINTRAEGE[39];
