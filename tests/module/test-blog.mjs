...
ok("der hierarchische Export (Issue 67) ist der neueste Eintrag",
  EINTRAEGE[0]?.id === "chg-20260813-01" && EINTRAEGE[0]?.issue === 67);
ok("der Export-Eintrag benennt den Wegfall des Planbild-Transports aussagewahr",
  /Planbilder/.test(EINTRAEGE[0]?.testbitte || "") && /entfallen/.test(EINTRAEGE[0]?.testbitte || ""));
ok("die kollisionsfreien Maßzahlen (Issue 59) folgen direkt danach",
  EINTRAEGE[1]?.id === "chg-20260812-10" && EINTRAEGE[1]?.issue === 59);
ok("die textfreie Eingabespalte in Modul 1 (Issue 69, zweite Kürzung) bleibt als dritter aktueller Eintrag erhalten",
  EINTRAEGE[2]?.id === "chg-20260812-09" && EINTRAEGE[2]?.issue === 69);
ok("die fokussierte Projektanlage (Issue 68) bleibt als vierter aktueller Eintrag erhalten",
  EINTRAEGE[3]?.id === "chg-20260812-08" && EINTRAEGE[3]?.issue === 68);
ok("die Wandbezeichnung in der Stückliste (Issue 70) bleibt als fünfter aktueller Eintrag erhalten",
  EINTRAEGE[4]?.id === "chg-20260812-07" && EINTRAEGE[4]?.issue === 70);
ok("die kompakte Eingabespalte in Modul 1 (Issue 69, erste Kürzung) bleibt als sechster aktueller Eintrag erhalten",
  EINTRAEGE[5]?.id === "chg-20260812-06" && EINTRAEGE[5]?.issue === 69);
ok("die aktive Wand im Geschosseditor (Issue 66) bleibt als siebter aktueller Eintrag erhalten",
  EINTRAEGE[6]?.id === "chg-20260812-05" && EINTRAEGE[6]?.issue === 66);
ok("die Blattreduktion zu Issue 61 bleibt als achter aktueller Eintrag erhalten",
  EINTRAEGE[7]?.id === "chg-20260812-04" && EINTRAEGE[7]?.issue === 61);
ok("der kompakte Lageplankopf zu Issue 59 bleibt als neunter aktueller Eintrag erhalten",
  EINTRAEGE[8]?.id === "chg-20260812-03" && EINTRAEGE[8]?.issue === 59);
ok("der Eintrag zu Issue 56 bleibt als zehnter aktueller Eintrag erhalten",
  EINTRAEGE[9]?.id === "chg-20260812-02" && EINTRAEGE[9]?.issue === 56);
