...
// #136 Kataloghoehen der Ausgleichssteine ([G-16]). Sie liegen — wie das Aktivierungs-Flag —
// NICHT im Vorspannblock, sondern sind eine Eigenschaft der Wand aus ihrer Produktauswahl
// ([P-13]), und sie muessen aus demselben Grund durch JEDE Iteration mitreisen: fielen sie hier
// weg, haelte der Core ab der ersten Iteration jeden Ausgleichsstein fuer einen Sonderzuschnitt,
// obwohl der Katalog die Hoehe fuehrt. FEHLT das Feld, bleibt es `null`: dann passt keine Hoehe,
// und es wird keine erfunden ([P-9]).
function ausgleichHoehenOf(vorg) { return vorg.ausgleich_stein_hoehen_mm || null; }
...
// Und aus demselben Grund `spannmutter_h_mm`/`spannmutter_sw_mm` (#97): Einbauhoehe und
// Schluesselweite des gewaehlten Spannmutter-Produkts. Sie gehen — wie die Schluesselweite der
// Kopplungsmutter und die Blechdicken — in KEINE Rechnung ein und werden allein AUSGEWIESEN,
// damit die Ausgaben die Mutter spaeter masstaeblich zeichnen koennen, ohne den Katalog zu
// lesen ([D-1]). Fielen sie hier weg, traege das Element nach der Auslegung keine Masse mehr,
// obwohl Modul 1 sie aus dem Katalog abgeleitet hat.
// Und aus demselben Grund `spannplatte_b_mm` (#97): die BREITE der gewaehlten Spannplatte. Sie
// geht — wie die Schluesselweiten und die Blechdicken — in KEINE Rechnung ein und wird allein
// AUSGEWIESEN, damit die Ausgaben die Platte spaeter masstaeblich zeichnen koennen, ohne den
// Katalog zu lesen ([D-1]). Nicht zu verwechseln mit `rod_kopf_zuschlag_mm` (der DICKE derselben
// Platte), die oben als echter Rechenwert steht. Fiele die Breite hier weg, traege das Element
...
export function autoAuslegung(vorg) {
  ...
  for (const sp of kand) {
    ...
    if (hit) {
      const wf = buildWall(vorg.name, vorg.length_mm, vorg.height_mm, vorg.openings || [], vorg.sides, psOf(vorg, { max_span_grid: sp, force_kN: hit.N }), stepsOf(vorg), interlocksOf(vorg), ausgleichOf(vorg), ausgleichHoehenOf(vorg));
  ...
  const wf = buildWall(vorg.name, vorg.length_mm, vorg.height_mm, vorg.openings || [], vorg.sides, psOf(vorg, { max_span_grid: kand[kand.length - 1], force_kN: last.N }), stepsOf(vorg), interlocksOf(vorg), ausgleichOf(vorg), ausgleichHoehenOf(vorg));
...
export function nachweisPruefen(vorg) {
  ...
  const w = buildWall(vorg.name, vorg.length_mm, vorg.height_mm, vorg.openings || [], vorg.sides, psOf(vorg, { max_span_grid: sp, force_kN: N }), stepsOf(vorg), interlocksOf(vorg), ausgleichOf(vorg), ausgleichHoehenOf(vorg));
